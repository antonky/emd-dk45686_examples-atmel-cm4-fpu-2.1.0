...
#define MAX_EXPECTED_PAYLOAD                                                   \
  (sizeof((*(DynProtocol_t *)(0)).decode_state_machine.tmp_buffer))
...
static inline int DynProtocol_decodeVect16QxToQy(const uint8_t *bytes,
                                                 unsigned len, int qxIn,
...
static inline int DynProtocol_encodeQxToQyVect16(const int32_t *in,
                                                 unsigned len, int qxIn,
...
static int16_t DynProtocol_getPayload(DynProtocol_t *self) {
  ...
  const enum DynProtocolEid cmdId =
      (enum DynProtocolEid)self->decode_state_machine.cmd_id;
  ...
  if (self->decode_state_machine.received_size == 0) {
    ...
    case EVENT_TYPE_RESP:
      ...
      case DYN_PROTOCOL_EID_SET_SENSOR_MMATRIX:
        return 1;
      case DYN_PROTOCOL_EID_GET_SENSOR_DATA:
        return 1 + 1 + 1 + 4 + 64; // rc + sensor_status + sensorId +
                                   // timestamp[0-3] + max_data_size
      case DYN_PROTOCOL_EID_GET_SW_REG:
        return 1;
      case DYN_PROTOCOL_EID_SET_SENSOR_CFG:
        return 1;
      case DYN_PROTOCOL_EID_GET_SENSOR_CFG:
        return 1 + 1 + 1 + 1 +
               64; // rc + sensor + size + cfg_type + max_data_size
      case DYN_PROTOCOL_EID_SET_SENSOR_BIAS:
        return 1; /* rc */
      case DYN_PROTOCOL_EID_GET_SENSOR_BIAS:
        return 1 + 1 + 1 +
               (3 * 2); /* rc + precision + cfg_type + bias (x,y,z) in Qx */
      case DYN_PROTOCOL_EID_WRITE_MEMS_REG:
        return 1;
      case DYN_PROTOCOL_EID_READ_MEMS_REG:
    ...
    case EVENT_TYPE_ASYNC:
      ...
      case DYN_PROTOCOL_EID_NEW_SENSOR_DATA:
        /* need at least two more byte to determine payload (sensor status +
         * sensor id) */
        return 2;
      default:
        break;
...
static int DynProtocol_decodeSensorEvent(DynProtocol_t *self,
  ...
  *sensor_status = (uint32_t)(buffer[idx] & 0x03);
  ...
  case DYN_PRO_SENSOR_TYPE_LINEAR_ACCELERATION:
    idx +=
        DynProtocol_decodeVect16QxToQy(&buffer[idx], 3, self->precision.acc, 16,
                                       (int32_t *)&sensor_data->data.u32[0]);
    sensor_data->base.meta_data = (uint32_t)buffer[idx++];
    break;
  case DYN_PRO_SENSOR_TYPE_MAGNETOMETER:
    idx += DynProtocol_decodeVect16QxToQy(&buffer[idx], 3,
                                          PROTOCOL_MAGNETOMETER_PRECISION, 16,
                                          (int32_t *)&sensor_data->data.u32[0]);
    sensor_data->base.meta_data = (uint32_t)buffer[idx++];
    break;
  case DYN_PRO_SENSOR_TYPE_UNCAL_MAGNETOMETER:
    ...
    break;
  case DYN_PRO_SENSOR_TYPE_GYROSCOPE:
    idx += DynProtocol_decodeVect16QxToQy(&buffer[idx], 3, self->precision.gyro,
                                          16,
                                          (int32_t *)&sensor_data->data.u32[0]);
    sensor_data->base.meta_data = (uint32_t)buffer[idx++];
    break;
  ...
  case DYN_PRO_SENSOR_TYPE_PRED_QUAT_0:
  case DYN_PRO_SENSOR_TYPE_PRED_QUAT_1:
    idx += DynProtocol_decodeVect16QxToQy(&buffer[idx], 4,
                                          PROTOCOL_QUATERNION_PRECISION, 30,
                                          (int32_t *)&sensor_data->data.u32[0]);
    sensor_data->base.meta_data = (uint32_t)buffer[idx++];
    break;
  case DYN_PRO_SENSOR_TYPE_ROTATION_VECTOR:
  case DYN_PRO_SENSOR_TYPE_GEOMAG_ROTATION_VECTOR:
    idx += DynProtocol_decodeVect16QxToQy(&buffer[idx], 4,
                                          PROTOCOL_QUATERNION_PRECISION, 30,
                                          (int32_t *)&sensor_data->data.u32[0]);
    idx += DynProtocol_decodeVect16QxToQy(
        &buffer[idx], 1, PROTOCOL_HEADING_ACCURACY_PRECISION, 16,
        (int32_t *)&sensor_data->data.u32[4]);
    sensor_data->base.meta_data = (uint32_t)buffer[idx++];
    break;
  case DYN_PRO_SENSOR_TYPE_HIGH_RATE_GYRO:
  ...
  case DYN_PRO_SENSOR_TYPE_RAW_GYROSCOPE: {
    ...
    sensor_data->data.u32[0] =
        (int32_t)((temp << 24) | (buffer[idx + 2] << 16) |
                  (buffer[idx + 1] << 8) | buffer[idx]);

    DYN_PROTOCOL_GET_SIGNED_VALUE(temp, buffer[idx + 5]);
    sensor_data->data.u32[1] =
        (int32_t)((temp << 24) | (buffer[idx + 5] << 16) |
                  (buffer[idx + 4] << 8) | buffer[idx + 3]);

    DYN_PROTOCOL_GET_SIGNED_VALUE(temp, buffer[idx + 8]);
    sensor_data->data.u32[2] =
        (int32_t)((temp << 24) | (buffer[idx + 8] << 16) |
  ...
  case DYN_PRO_SENSOR_TYPE_TEMPERATURE:
  case DYN_PRO_SENSOR_TYPE_AMBIENT_TEMPERATURE:
    DynProtocol_decodeVect16QxToQy(&buffer[idx], 1,
                                   PROTOCOL_TEMPERATURE_PRECISION, 16,
                                   (int32_t *)&sensor_data->data.u32[0]);
    break;
  case DYN_PRO_SENSOR_TYPE_RAW_TEMPERATURE:
  ...
  case DYN_PRO_SENSOR_TYPE_ORIENTATION:
    idx += DynProtocol_decodeVect16QxToQy(&buffer[idx], 3,
                                          PROTOCOL_ORIENTATION_PRECISION, 16,
                                          (int32_t *)&sensor_data->data.u32[0]);
    sensor_data->base.meta_data = (uint32_t)buffer[idx++];
    break;
  case DYN_PRO_SENSOR_TYPE_LIGHT:
  ...
  case DYN_PRO_SENSOR_TYPE_CUSTOM7: {
    ...
    // hard code the payload size until payload has a fixed value for custom
    // sensors
  ...
  case DYN_PRO_SENSOR_TYPE_CUSTOM_PRESSURE:
    for (i = 0; i < 4; i++) {
      sensor_data->data.u32[i] =
          inv_dc_little8_to_int32(&buffer[idx]); // raw pressure
      idx += 4;
    }
    break;
  ...
  case DYN_PRO_SENSOR_TYPE_HEART_RATE: {
    // ppm
    idx += DynProtocol_decodeVect16QxToQy(&buffer[idx], 1,
                                          PROTOCOL_HRM_PRECISION, 16,
                                          (int32_t *)&sensor_data->data.u32[0]);
...
static int DynProtocol_decodePktCommand(DynProtocol_t *self,
  ...
  if (self->decode_state_machine.cmd_id == DYN_PROTOCOL_EID_PROTOCOLVERSION) {
    ...
    case DYN_PROTOCOL_EID_SET_SENSOR_MMATRIX: {
      ...
      for (i = 0; i < 9; i++)
        ((DynProtocolConfigReferenceFrame_t *)&edata->d.command.cfg)
            ->matrix[i] = (int32_t)inv_dc_little8_to_int32(&buf[2 + i * 4]);
      break;
    }
...
static int DynProtocol_decodePktResponse(DynProtocol_t *self,
  ...
  case DYN_PROTOCOL_EID_PROTOCOLVERSION:
  case DYN_PROTOCOL_EID_GET_FW_INFO:
    memcpy(edata->d.response.version, buf,
           sizeof(edata->d.response.version) - 1);
  ...
  case DYN_PROTOCOL_EID_GET_SENSOR_DATA: {
    ...
    if (edata->d.response.sensorData.rc == 0)
      rc = DynProtocol_decodeSensorEvent(self, &buf[1], len, edata,
                                         DYN_PROTOCOL_ETYPE_RESP);
    break;
  }
  ...
  case DYN_PROTOCOL_EID_GET_SENSOR_BIAS:
    ...
    DynProtocol_decodeVect16QxToQy(
        &buf[3], 3, precision, 16,
        &((DynProtocolConfigOffset_t *)&edata->d.response.sensorcfg.cfg)
             ->vect[0]);
    break;
...
static int DynProtocol_decodePktAsync(DynProtocol_t *self,
  ...
  switch (self->decode_state_machine.cmd_id) {
  case DYN_PROTOCOL_EID_NEW_SENSOR_DATA:
    return DynProtocol_decodeSensorEvent(self, buf, len, edata,
...
static int DynProtocol_doProcess(DynProtocol_t *self) {
  ...
  if (rc == 0) {
    DynProtocol_callEventCB(
        self, etype, (enum DynProtocolEid)self->decode_state_machine.cmd_id,
        &edata);
    return 1;
  } else {
...
static int DynProtocol_encodeSensorEvent(DynProtocol_t *self,
                                         const DynProtocolEdata_t *edata,
                                         uint8_t *outBuffer,
  ...
  case DYN_PRO_SENSOR_TYPE_LINEAR_ACCELERATION:
    if (maxBufferSize < 6 + 1)
      goto error_size;
    idx += DynProtocol_encodeQxToQyVect16((int32_t *)&sensor_data->data.u32[0],
                                          3, 16, self->precision.acc,
                                          &outBuffer[idx]);
    /* report accuracy */
    outBuffer[idx++] = (uint8_t)(sensor_data->base.meta_data &
                                 DYN_PROTOCOL_DATA_ACCURACY_MASK);
    break;
  ...
  case DYN_PRO_SENSOR_TYPE_MAGNETOMETER:
    ...
    idx += DynProtocol_encodeQxToQyVect16(
        (int32_t *)&sensor_data->data.u32[0], 3, 16,
  ...
  case DYN_PRO_SENSOR_TYPE_UNCAL_MAGNETOMETER:
    ...
    // bias
    idx += DynProtocol_encodeQxToQyVect16(
        (int32_t *)&sensor_data->data.u32[3], 3, 16,
        PROTOCOL_MAGNETOMETER_PRECISION, &outBuffer[idx]);
    // accuracy
    outBuffer[idx++] = (uint8_t)(sensor_data->base.meta_data &
                                 DYN_PROTOCOL_DATA_ACCURACY_MASK);
  ...
  case DYN_PRO_SENSOR_TYPE_UNCAL_GYROSCOPE:
    ...
    idx += DynProtocol_encodeQxToQyVect16((int32_t *)&sensor_data->data.u32[0],
                                          ...
                                          &outBuffer[idx]);
    // bias
    idx += DynProtocol_encodeQxToQyVect16((int32_t *)&sensor_data->data.u32[3],
                                          3, 16, self->precision.gyro,
                                          &outBuffer[idx]);
    // accuracy
    outBuffer[idx++] = (uint8_t)(sensor_data->base.meta_data &
  ...
  case DYN_PRO_SENSOR_TYPE_PRED_QUAT_1:
    if (maxBufferSize < 9)
      goto error_size;
    idx += DynProtocol_encodeQxToQyVect16((int32_t *)&sensor_data->data.u32[0],
                                          4, 30, PROTOCOL_QUATERNION_PRECISION,
                                          &outBuffer[idx]);
    outBuffer[idx++] = (uint8_t)(sensor_data->base.meta_data &
                                 DYN_PROTOCOL_DATA_ACCURACY_MASK);
    break;
  ...
  case DYN_PRO_SENSOR_TYPE_GEOMAG_ROTATION_VECTOR:
    ...
    // accuracy in q16
    idx += DynProtocol_encodeQxToQyVect16(
        (int32_t *)&sensor_data->data.u32[4], 1, 16,
  ...
  case DYN_PRO_SENSOR_TYPE_AMBIENT_TEMPERATURE:
    idx += DynProtocol_encodeQxToQyVect16((int32_t *)&sensor_data->data.u32[0],
                                          1, 16, PROTOCOL_TEMPERATURE_PRECISION,
                                          &outBuffer[idx]);
    break;
  ...
  case DYN_PRO_SENSOR_TYPE_ORIENTATION: {
    if (maxBufferSize < 6 + 1)
      goto error_size;
    idx += DynProtocol_encodeQxToQyVect16((int32_t *)&sensor_data->data.u32[0],
                                          3, 16, PROTOCOL_ORIENTATION_PRECISION,
                                          &outBuffer[idx]);
    outBuffer[idx++] = (uint8_t)(sensor_data->base.meta_data &
                                 DYN_PROTOCOL_DATA_ACCURACY_MASK);
    break;
  ...
  case DYN_PRO_SENSOR_TYPE_CUSTOM7: {
    ...
    maxBufferSize--;
    // let's make sure that size to be copied is not bigger than size left in
    // outBuffer
    if (sensor_data->base.meta_data > maxBufferSize)
      goto error_size;

    memcpy(&outBuffer[idx], sensor_data->data.u8, sensor_data->base.meta_data);
    // set hardly the payload size until payload has a fixed value for custom
    // sensors
    idx += sizeof(sensor_data->data.u8);
    break;
  }
  ...
  case DYN_PRO_SENSOR_TYPE_CUSTOM_PRESSURE: {
    ...
    for (i = 0; i < 4; i++) {
      inv_dc_int32_to_little8(sensor_data->data.u32[i],
                              &outBuffer[idx]); // raw pressure
      idx += 4;
    }
    break;
  }
...
static int DynProtocol_checkFrameSize(DynProtocol_t *self) {
  ...
  /*
   * expected_size is the payload expected size (not the total frame expected
   * size). it should respect this condititon: current_frame_size >= 1 byte
   * (GID) + 1 byte (CID) + expected_size note : current_frame_size ==
   * UINT16_MAX means check on frame size disabled (default).
   */
  if (self->decode_state_machine.current_frame_size != UINT16_MAX) {
    if (self->decode_state_machine.current_frame_size <
      ...
      INV_MSG(INV_MSG_LEVEL_ERROR,
              "DynProtocol: Frame size error. size=%dB (expected %dB)",
...
void DynProtocol_init(DynProtocol_t *self, DynProtocolEvent_cb event_cb,
...
int DynProtocol_setPrecision(DynProtocol_t *self, int sensor,
                             uint8_t precision) {
  int rc = 0;
  switch (sensor) {
  case DYN_PRO_SENSOR_TYPE_ACCELEROMETER:
...
int DynProtocol_processPktByte(DynProtocol_t *self, uint8_t rcvByte) {
  ...
  case PROTOCOL_STATE_PAYLOAD: {
    ...
    if (self->decode_state_machine.received_size >= MAX_EXPECTED_PAYLOAD)
      ...
      self->decode_state_machine
          .tmp_buffer[self->decode_state_machine.received_size] = rcvByte;

    self->decode_state_machine.received_size++;
    if (self->decode_state_machine.received_size ==
        self->decode_state_machine.expected_size)
      /* update expected payload, in case actual payload cannot be determined
       * using only CID */
      self->decode_state_machine.expected_size = DynProtocol_getPayload(self);
...
int DynProtocol_encodeCommand(DynProtocol_t *self, enum DynProtocolEid eid,
  ...
  *outBufferSize = 0;
  ...
  outBuffer[idx++] |=
      (DYN_PROTOCOL_GROUP_ID & ~EVENT_TYPE_MASK); // Set group ID
  ...
  case DYN_PROTOCOL_EID_SET_SENSOR_MMATRIX:
    ...
    outBuffer[idx++] = (uint8_t)edata->sensor_id;
    outBuffer[idx++] =
        (uint8_t)((DynProtocolConfigReferenceFrame_t *)&edata->d.command.cfg)
            ->base.type;
    for (i = 0; i < 9; i++) {
  ...
  case DYN_PROTOCOL_EID_SET_SENSOR_CFG:
    ...
    case DYN_PROTOCOL_CONFIG_TYPE_OFFSET:
      precision = DynProtocol_getPrecision(self, edata->sensor_id);
      DynProtocol_encodeQxToQyVect16((int32_t *)&edata->d.command.cfg.buffer[0],
                                     3, 16, precision, &outBuffer[idx]);
      break;
    default:
      ...
      if ((uint32_t)(maxBufferSize - idx) < edata->d.command.cfg.base.size)
        goto error_size;
      memcpy(&outBuffer[idx], edata->d.command.cfg.buffer,
  ...
  case DYN_PROTOCOL_EID_SET_SENSOR_BIAS:
    ...
    precision = DynProtocol_getPrecision(self, edata->sensor_id);
    idx += DynProtocol_encodeQxToQyVect16(
        &((DynProtocolConfigOffset_t *)&edata->d.command.cfg)->vect[0], 3, 16,
  ...
  case DYN_PROTOCOL_EID_TRANSFER_BUFFER:
    ...
    memcpy(&outBuffer[idx], edata->d.command.buffer.address,
           edata->d.command.buffer.size);
    idx += 512;
    break;

  default:
    INV_MSG(INV_MSG_LEVEL_ERROR,
            "DynProtocol: Unexpected argument for encode_command()");
    return -1;
  }
  ...
  *outBufferSize = 0;
  ...
  case DYN_PROTOCOL_EID_GET_SENSOR_DATA: {
    if ((maxBufferSize - idx) <
        (1 + 1 + 1 + 4 +
         64)) // need room for rc+sensor_status+sensorid+timestamp+data
      goto error_size;
    outBuffer[idx++] = edata->d.response.sensorData.rc;
    if (DynProtocol_encodeSensorEvent(self, edata, &outBuffer[idx],
                                      maxBufferSize - idx,
                                      DYN_PROTOCOL_ETYPE_RESP) == -1)
      goto error_arg;

    // GET_SENSOR_DATA response frame size is fixed:
    //  EVT_TYPE + EVT_ID + RC + SENSOR_STATUS + SENSORID + TIMESTAMP[0-3] +
    //  MAX_SENSOR_EVENT_DATA_SIZE (arbitrary fixed to 64B)
    idx = 1 + 1 + 1 + 1 + 1 + 4 + 64;
    break;
  }
  ...
  case DYN_PROTOCOL_EID_GET_SENSOR_CFG: {
    ...
    case DYN_PROTOCOL_CONFIG_TYPE_OFFSET:
      outBuffer[idx++] = (uint8_t)edata->sensor_id;
      precision = DynProtocol_getPrecision(self, edata->sensor_id);
      DynProtocol_encodeQxToQyVect16(
          (int32_t *)&edata->d.response.sensorcfg.cfg.buffer[0], 3, 16,
          precision, &outBuffer[idx]);
      break;
    default:
      // we don't know what is the value of cfg.size so let's make sure
      // there is enough room in outBuffer.
      if ((uint32_t)(maxBufferSize - idx) <
...
int DynProtocol_encodeAsync(DynProtocol_t *self, enum DynProtocolEid eid,
  ...
  *outBufferSize = 0;
  ...
  switch (eid) {
  case DYN_PROTOCOL_EID_NEW_SENSOR_DATA: {
    len = DynProtocol_encodeSensorEvent(self, edata, &outBuffer[idx],
                                        maxBufferSize - idx,
                                        DYN_PROTOCOL_ETYPE_ASYNC);
    if (len == -1)
      goto error_arg;
...
error_arg:
  INV_MSG(INV_MSG_LEVEL_ERROR,
          "DynProtocol: Unexpected argument for encode_async()");
  return -1;

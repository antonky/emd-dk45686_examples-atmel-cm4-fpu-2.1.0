...
typedef struct {
  ...
  fifo_config0_fifo_mode_t
      fifo_mode; /**< Current fifo mode. Required by AN-000364 */
  ...
  uint8_t accel_baseline_found; /**< Flag indicating accel baseline has been
                                   found */
  uint8_t
      gyro_baseline_found; /**< Flag indicating gyro baseline has been found */
  uint8_t temp_baseline_found; /**< Flag indicating temperature baseline has
                                  been found */

#if INV_IMU_FSYNC_SUPPORTED
  /* Variables related to FSYNC tag */
  fsync_config0_ap_fsync_sel_t
      fsync_tag; /**< Keeps track of FSYNC tag in sensor data regs */
#endif
} inv_imu_adv_var_t;
...
/** @brief Configures INT2 pin for the requested usage (INT2, FSYNC, CLKIN or
 * DRDY_INTR). It also disables AUX2 in pad scenario if AUX2 is supported for
 * current variant.
 *  @param[in] s      Pointer to device.
 ...
 *  @param[in] s           Pointer to device.
 *  @param[in] sensor_tag  Indicates which sensor data register LSB should be
 * set when a FSYNC event is detected.
 *  @return                0 on success, negative value on error.
 */
 ...
 *  In details it:
 *     - enables fsync
 *     - enables timestamp to registers. Once fysnc is enabled fsync counter is
 * pushed to fifo instead of timestamp. So timestamp is made available in
 * registers. Note that this increase power consumption.
 ...
 *  In details it:
 *     - disables fsync
 *     - disables timestamp to registers. Once fysnc is disabled  timestamp is
 * pushed to fifo instead of fsync counter. So in order to decrease power
 * consumption, timestamp is no more available in registers.
...
/** @brief Read all registers containing data (temperature, accelerometer and
 * gyroscope). It will then call `sensor_event_cb` function provided in the
 * `inv_imu_device_t` for each packet.
 *  @param[in] s  Pointer to device.
 *  @return       0 on success, negative value on error.
 */
 ...
 *         sensor event containing packet data and validity information. Then it
 * calls sensor_event_cb funtion passed in parameter of inv_imu_init function
 * for each packet.
 *  @param[in] s           Pointer to device.
...
/** @brief Converts accel_config0_accel_odr_t or gyro_config0_gyro_odr_t enums
 * to period expressed in us
 *  @param[in] odr_bitfield An accel_config0_accel_odr_t or
 * gyro_config0_gyro_odr_t enum
 ...
 *  @param[in] s         Pointer to device.
 *  @param[in] wom_x_th  Threshold for X axis with 1g/256 resolution (wom_x_th =
 * mg * 256 / 1000).
 *  @param[in] wom_y_th  Threshold for Y axis with 1g/256 resolution (wom_y_th =
 * mg * 256 / 1000).
 *  @param[in] wom_z_th  Threshold for Z axis with 1g/256 resolution (wom_z_th =
 * mg * 256 / 1000).
 *  @param[in] wom_int   Mode used to generate interrupt (AND/OR).
 *  @param[in] wom_dur   Number of overthreshold events to wait before
 ...
 *  As a consequence Fifo water-mark interrupt is disabled to only trigger WoM
 interrupts.
 *  To have good performance, it's recommended to set accelerometer ODR (Output
 Data Rate) to 20ms

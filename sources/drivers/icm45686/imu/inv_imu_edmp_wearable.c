...
#include "imu/edmp_ram_extended_features_image.h"
  ...
  status |= inv_imu_write_sram(s, (uint32_t)EDMP_RAM_FEATURE_PRGM_RAM_BASE,
                               sizeof(ram_img), ram_img);
...
int inv_imu_edmp_b2s_set_parameters(
  ...
  status |= INV_IMU_WRITE_EDMP_SRAM(
      s, EDMP_B2S_MOUNTING_MATRIX, (uint8_t *)&b2s_params->b2s_mounting_matrix);

...
struct uart_mapping {
  ...
  uint32_t uart_periph_id; /** < Peripheral ID for targeted UART as defined by
                              ATMEL specification */
  struct uart_gpio uart_gpio[4]; /** < GPIO configuration for RXD/TXD/RTS/CTS.
                                                    Gpio whose configuration
                                    port is 0xFFFFFFFF are ignored */
  volatile inv_uart_state_t uart_tx_state;
  inv_uart_state_t uart_rx_state;
  volatile uint8_t *uart_rx_buffer; /** < Pointer to the memory region used to
                                       store Rx bytes */
  volatile uint8_t *uart_tx_buffer;
  uint16_t uart_rx_buffer_size; /** < The size of the memory region used to
                                   store Rx bytes */
  uint16_t uart_tx_buffer_size;
  volatile uint16_t uart_rx_buffer_tail; /** < Index of the last read byte from
                                            the uart_rx_buffer */
  void (*tx_done_cb)(
      void *context); /** < Callback executed by UART DMA tx IRQ handler */
...
static struct uart_mapping um[2] = {
    ...
    {.uart_ip = USART0,
     ...
     .uart_gpio = {{IOPORT_PIOA, PIO_PA9A_RXD0, IOPORT_MODE_MUX_A},
...
int inv_uart_init(inv_uart_init_struct_t *uart_init) {
  ...
  /* Configure GPIO pins */
  for (i = 0; i < (uint32_t)(sizeof(um[uart].uart_gpio) /
                             sizeof(um[uart].uart_gpio[0]));
       i++) {
  ...
  if (uart_init->flow_ctrl != INV_UART_FLOW_CONTROL_NONE) {
    if (uart == INV_UART_SENSOR_CTRL)
      um[INV_UART_SENSOR_CTRL].uart_ip->US_MR =
          (um[INV_UART_SENSOR_CTRL].uart_ip->US_MR & ~US_MR_USART_MODE_Msk) |
  ...
  /* Trigger a DMA RX.
   * Note: DMA is in circular mode. refer to the device datasheet for further
   * details.
   */
...
int inv_uart_tx_txfer(inv_uart_num_t uart, inv_uart_tx_transfer_t *txfer) {
  ...
  if (uart_tx_state == INV_UART_STATE_IDLE) {
    if (txfer->len > um[uart].uart_tx_buffer_size) {
      ...
      while (((pdc_read_status(um[uart].uart_pdc) & PERIPH_PTCR_TXTEN) == 0) &&
             (timeout-- > 0))
        ;
      if (timeout == 0) {
...
int inv_uart_available(inv_uart_num_t uart) {
  uint16_t head =
      um[uart].uart_rx_buffer_size - pdc_read_rx_counter(um[uart].uart_pdc);
...
inv_uart_get_flow_control_configuration(inv_uart_num_t uart) {
  if ((um[uart].uart_ip->US_MR & US_MR_USART_MODE_Msk) ==
      US_MR_USART_MODE_HW_HANDSHAKING)
...
static int uart_dma_rx(inv_uart_num_t uart) {
  ...
  if (um[uart].uart_rx_state == INV_UART_STATE_IDLE) {
    ...
    /* Initialize PDC (DMA) transfer address and size. Set same values for next
     * transfer as RX is used in circular mode (cf atmel SAM55G datasheet
     * §21.5.3).
     */
    ...
    timeout = 1000;
    while (((pdc_read_status(um[uart].uart_pdc) & PERIPH_PTCR_RXTEN) == 0) &&
           (timeout-- > 0))
      ;
    if (timeout == 0) {
      um[uart].uart_rx_state = INV_UART_STATE_IDLE;
...
void USART0_IRQ_HANDLER(void) { commonUSART_IRQ_HANDLER(INV_UART_SENSOR_CTRL); }

void USART7_IRQ_HANDLER(void) { commonUSART_IRQ_HANDLER(INV_UART_LOG); }

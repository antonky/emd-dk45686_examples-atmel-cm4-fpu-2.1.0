 ...
 *  @return          0 on success, negative value on error.
 */
#define INV_IMU_READ_EDMP_SRAM(s, name, val)                                   \
  inv_imu_read_sram(s, (uint32_t)name, name##_SIZE, val)
 ...
 *  @note Refer to the datasheet for details on how to configure these
 * parameters.
 */
typedef struct {
  /* Pedometer */
...
typedef struct {
   ...
   *         Number of samples between two steps with u6.2 format (8-bits
   * unsigned in Q2). cadency (steps/s) = EDMP_ODR_HZ / (step_cadence * 0.25)
   */
 ...
 *  @warning If inv_imu_edmp_init_apex() was already called, application should
 * call `inv_imu_edmp_recompute_apex_decimation()` afterwards if APEX algorithms
 * are to be run.
 *  @param[in] s         Pointer to device.
...
/** @brief Initialize EDMP APEX algorithms. This function should be called
 * before calling any other function (except for `inv_imu_edmp_set_frequency`).
 *  @warning This function will power-up the SRAM. For power consumption
 * consideration, you can manually call `inv_imu_adv_power_down_sram` if you
 * don't need to preserve SRAM content.
 *  @warning This function will reset all interrupt masks previously set with
 *           `inv_imu_edmp_unmask_int_src` and exit with
 * `EDMP_INT_SRC_ACCEL_DRDY_MASK` unmasked on `INV_IMU_EDMP_INT0`.
 *  @param[in] s  Pointer to device.
 *  @return       0 on success, negative value on error or if EDMP is enabled.
 */
int inv_imu_edmp_init_apex(inv_imu_device_t *s);

/** @brief Recompute EDMP APEX algorithms internal decimator based on new EDMP
 output Data Rate configured with `inv_imu_edmp_set_frequency`.
 *  @warning It is up to application level to save/restore previously configured
 APEX parameters,
 *           if any, with `inv_imu_edmp_set_apex_parameters`.
 *  @warning EDMP must be disabled before calling this function.
 *  @warning This function will reset all interrupt masks previously set with
 *           `inv_imu_edmp_unmask_int_src` and exit with
 `EDMP_INT_SRC_ACCEL_DRDY_MASK` unmasked on
 *           `INV_IMU_EDMP_INT0`.
 *  @param[in] s         Pointer to device.
 *  @return              0 on success, negative value on error or if EDMP is
 enabled.
 */
 ...
 *  @warning This function should be called only when all EDMP algorithms are
 * disabled.
 *  @param[in] s  Pointer to device.
 ...
 *  @return           0 on success, negative value on error.
 *  @retval           INV_IMU_ERROR_EDMP_BUF_EMPTY if step count buffer is
 * empty.
 */
...
/** @brief  Mask requested interrupt sources for edmp interrupt line passed in
 * parameter.
 *  @param[in] s            Pointer to device.
 *  @param[in] edmp_int_nb  EDMP input interrupt line number that should be
 * configured.
 *  @param[in] int_mask     Interrupt sources to mask.
 *  @return                 0 on success, negative value on error.
 */
int inv_imu_edmp_mask_int_src(inv_imu_device_t *s,
                              inv_imu_edmp_int_t edmp_int_nb, uint8_t int_mask);

/** @brief  Unmask requested interrupt sources for edmp interrupt line passed in
 * parameter.
 *  @param[in] s            Pointer to device.
 *  @param[in] edmp_int_nb  EDMP input interrupt line number that should be
 * configured.

...
static void get_time_and_milliseconds_synced(Rtc *p_rtc, uint32_t *pul_hour,
                                             uint32_t *pul_minute,
                                             uint32_t *pul_second,
                                             uint32_t *pul_millisecond);
...
static uint64_t rtc_timer_get_timestamp_us(void) {
  ...
  get_time_and_milliseconds_synced(RTC, &rtc_hour, &rtc_minute, &rtc_second,
                                   &counter_subsec);
  ...
  subsecond_us = (uint64_t)rtc_hour * 3600ULL * 1000000ULL +
                 ...
                 (uint64_t)rtc_second * 1000000ULL + base_us;
 ...
 * Required to read RTC_TIMR and RTC_MSR simultaneously to prevent out of sync
 * issue A while loop is required to ensure the data is stable
 */
static void get_time_and_milliseconds_synced(Rtc *p_rtc, uint32_t *pul_hour,
                                             uint32_t *pul_minute,
                                             uint32_t *pul_second,

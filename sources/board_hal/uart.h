...
typedef enum uart_num {
  INV_UART_SENSOR_CTRL, /**< Just a fancier and hw independent name for UART0.
                           It is used < to send command to/read data from
                           sensors. < On samg55 it is mapped on CN6 USB port
                           through FTDI. < UART0 has hw handshake capabilities.
                         */
  INV_UART_LOG, /**< Just a fancier and hw independent name for UART7. It is
                   used < for SW traces. < On samg55 it is mapped through EDBG
                   USB port.
                          < */
} inv_uart_num_t;
...
typedef struct {
  ...
  volatile uint16_t
      *idx; /** < Pointer to the index of the first data byte to read */
} inv_uart_rx_context_t;

/** @brief UART initialization structure.
 *
 * Such a structure should be filled-in and passed to the uart_init function in
 * order to initialize the uart peripheral. All fields need to be initialized in
...
typedef struct inv_uart_init_struct {
  ...
  uint8_t *tx_buffer; /** < Pointer to the start of the memory area allocated
                         for the UART TX FIFO */
  uint8_t *rx_buffer; /** < Pointer to the start of the memory area allocated
                         for the UART RX FIFO */
  uint16_t tx_size;   /** < Buffer size for UART TX FIFO */
  uint16_t rx_size;   /** < Buffer size for UART RX FIFO.
                                    Choose wisely this value, as overflow will
                         lead to byte loss */
  int baudrate; /** < UART baudrate speed. For a list of supported baudrates
                   refer to the device datasheet */
  inv_uart_flow_control_t
      flow_ctrl; /** < Flow control as defined by uart_flow_control_t enum
                           Note: UART2 and UART6 support only
                    UART_FLOW_CONTROL_NONE */
  void (*tx_done_cb)(void *context); /** < Callback executed when a DMA TX
 ...
 *              INV_UART_ERROR if for an unknown reason the initialization
 * failed INV_UART_ERROR_BAD_ARG if the uart_init_structure was incorrectly
 * filled-in
 */
 ...
 *              INV_UART_ERROR if for an unknown reason the requested
 * transaction did not start INV_UART_ERROR_BAD_ARG if one of the arguments is
 * unsupported
 ...
 * and does no have to be NULL-terminated.
 *  @param[in]  uart UART peripheral
 *  @param[in]  txfer Pointer to a inv_uart_tx_transfer_t structure that
 * contains information on the transfer
 *  @return     0 on success
 *              INV_UART_ERROR_MEMORY if the internal buffers are full
 *              INV_UART_ERROR_BUSY if another transaction is already ongoing
 *              INV_UART_ERROR if for an unknown reason the requested
 ...
 *  @param[in]  uart UART peripheral
 *  @return     Current flow control configuration as defined by the
 * inv_uart_flow_control_t enum
 */
inv_uart_flow_control_t
inv_uart_get_flow_control_configuration(inv_uart_num_t uart);

/** @brief      Returns the state of the TX module
 *  @param[in]  uart UART peripheral
 *  @return     The state of the TX module as defined by the inv_uart_state_t
 * enum
 */
 ...
 *  @param[in]  uart UART peripheral
 *  @return     The state of the RX module as defined by the inv_uart_state_t
 * enum
 */

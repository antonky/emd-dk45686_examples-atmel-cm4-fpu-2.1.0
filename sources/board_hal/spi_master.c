...
struct spi_mapping {
  ...
  uint32_t clk_pol;
  uint32_t clk_ph;
  uint8_t dlybs;  // delay before SPCK. See SPI_CSR register in the device
                  // datasheet for more info
  uint8_t dlybct; // delay between consecutive transfers. See SPI_CSR register
                  // in the device datasheet for more info
...
struct spi_mapping sm[INV_SPI_MAX] = {
    ...
    {.p_spi = SPI5,
     ...
     .rx_dest_addr = 0,
     .rx_len = 0,
     // For systems that send a leading zero with each read.
     .leading_zeros = 0},
    /*
...
static void spi_master_irq_handler(unsigned spi_num) {
  ...
  pdc_disable_transfer(sm[spi_num].p_pdc,
                       PERIPH_PTCR_RXTDIS | PERIPH_PTCR_TXTDIS);

  NVIC_ClearPendingIRQ(sm[spi_num].irqn);

  if (sm[spi_num].p_spi->SPI_SR & SPI_SR_RXBUFF) {
    memcpy(sm[spi_num].rx_dest_addr, &sm[spi_num].rx_buffer[1],
           sm[spi_num].rx_len);
...
void inv_spi_master_init(unsigned spi_num, uint32_t speed_hz) {
  ...
  spi_configure_cs_behavior(sm[spi_num].p_spi, sm[spi_num].chip_sel,
                            SPI_CS_RISE_NO_TX);

  spi_set_peripheral_chip_select_value(sm[spi_num].p_spi, sm[spi_num].chip_sel);

  spi_set_clock_polarity(sm[spi_num].p_spi, sm[spi_num].chip_sel,
                         sm[spi_num].clk_pol);
  spi_set_clock_phase(sm[spi_num].p_spi, sm[spi_num].chip_sel,
                      sm[spi_num].clk_ph);
  spi_set_bits_per_transfer(sm[spi_num].p_spi, sm[spi_num].chip_sel,
                            SPI_CSR_BITS_8_BIT);
  spi_set_baudrate_div(sm[spi_num].p_spi, sm[spi_num].chip_sel,
                       (sysclk_get_peripheral_hz() / speed_hz));
  spi_set_transfer_delay(sm[spi_num].p_spi, sm[spi_num].chip_sel,
                         sm[spi_num].dlybs, sm[spi_num].dlybct);
...
unsigned long inv_spi_master_read_register(unsigned spi_num,
  ...
  /* Deactivate Irq during buffer write*/
  inv_disable_irq();
  pdc_spi_packet.ul_addr = (uint32_t)&sm[spi_num].rx_buffer[0];

  /* For systems that prepend the read buffer with a leading zero, add 1 to
   * length */
  pdc_spi_packet.ul_size = len + 1 + sm[spi_num].leading_zeros;
  pdc_rx_init(sm[spi_num].p_pdc, &pdc_spi_packet, NULL);
  ...
  if (sm[spi_num].transfer_done_cb == 0) {
    /* Enable the RX and TX PDC transfer requests */
    pdc_enable_transfer(sm[spi_num].p_pdc,
    ...
    /* If there is a leading zero, the data starts at byte 2 */
    memcpy(value, &sm[spi_num].rx_buffer[1 + sm[spi_num].leading_zeros], len);
  } else { /* Note: If async mode is used with a leading zero byte,
          the zero byte will need to be stripped in the callback. */
...
static void spi_master_spi_abort_async(unsigned spi_num) {
  ...
  if (pdc_read_rx_counter(sm[spi_num].p_pdc) ||
      pdc_read_rx_next_counter(sm[spi_num].p_pdc)) {
...
void spi_master_set_leading_zeros(unsigned spi_num, uint8_t num_zeros) {
  if (num_zeros > 1) /* Valid options are 0 or 1 */
    return;
  sm[spi_num].leading_zeros = num_zeros;
}

void FLEXCOM5_Handler(void) { spi_master_irq_handler(invn_spi_num); }

...
/** \brief  Macro to declare a ring buffer
        ...
        \param[in] size 	number of items that can contain the ring buffer
                                                To improve speed, size should be
   a power of 2
*/
...
/** \brief  Macro to declare a volatile ring buffer, i.e. modified within an
   interrupt context
        \param[in] type		type of item contained in the ring buffer
        \param[in] size 	number of items that can contain the ring buffer
                                                To improve speed, size should be
...
/** \brief  Macro to declare a volatile ring buffer, i.e. modified within an
   ...
   a power of 2
        \param[in] type		type of item contained in the ring buffer
*/
#define RINGBUFFER_VOLATILE(name, size, type)                                  \
  RINGBUFFER_VOLATILE_DECLARE(type, size) name
...
/** \brief  Macro to get maximum size of a volatile ring buffer, i.e. modified
...
/** \brief   Macro to get current size of a volatile ring buffer, i.e. modified
   within an interrupt context
        \param[in] rb 		pointer to the ring buffer
        \return  	current number of items hold in the ringbuffer
...
/** \brief   Macro to check if a ring buffer is full
        \param[in] rb 		pointer to the ring buffer
        \return  	1 if there is no slot left in the ring buffer, 0
   otherwise
*/
#define RINGBUFFER_FULL(rb) (RINGBUFFER_SIZE(rb) == RINGBUFFER_MAXSIZE(rb))

/** \brief   Macro to check if a volatile ring buffer, i.e. modified within an
   interrupt context, is full
        \param[in] rb 		pointer to the ring buffer
        \return  	1 if there is no slot left in the ring buffer, 0
   otherwise
        \warning it is advised to put this in a critical section
...
/** \brief   Macro to get number of available slot in a volatile ring buffer,
   i.e. modified within an interrupt context
        \param[in] rb 		pointer to the ring buffer
        \return  	number of empty slot in the ring buffer
        \warning it is advised to put this in a critical section
*/
...
/** \brief   Macro to clear a volatile ring buffer, i.e. modified within an
   interrupt context
        \param[in] rb 		pointer to the ring buffer
        \warning it is advised to put this in a critical section
*/
...
/** \brief  Increment write counter
                        Actually performed a push (assuming data were already
   copied)
        \param[in] rb 			pointer to the ring buffer
        \warning There is no error checking done.
*/
...
/** \brief   Macro to pop an item from a volatile ring buffer (data is not
   copied)
        \param[in] rb 			pointer to the ring buffer
        \warning There is no error checking done.
        You must check for emptiness before popping data

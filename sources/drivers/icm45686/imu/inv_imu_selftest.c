...
static int
set_selftest_parameters(inv_imu_device_t *s,
                        const inv_imu_selftest_parameters_t *st_params);
static int run_internal_selftest(inv_imu_device_t *s);
static int get_selftest_output(inv_imu_device_t *s,
                               const inv_imu_selftest_parameters_t *st_params,
                               inv_imu_selftest_output_t *st_output);

/* API implementation */

int inv_imu_selftest_init_params(inv_imu_device_t *s,
...
int inv_imu_selftest(inv_imu_device_t *s,
                     const inv_imu_selftest_parameters_t *st_params,
                     inv_imu_selftest_output_t *st_output) {
  int rc = INV_IMU_OK;
...
set_selftest_parameters(inv_imu_device_t *s,
  ...
  tmp_stc_params |=
      (st_params->accel_en ? SELFTEST_ACCEL_EN : SELFTEST_ACCEL_DIS);
  tmp_stc_params |= (st_params->gyro_en ? SELFTEST_GYRO_EN : SELFTEST_GYRO_DIS);
  tmp_stc_params |=
      (uint32_t)(st_params->accel_limit & SELFTEST_ACCEL_THRESH_MASK);
  tmp_stc_params |=
      (uint32_t)(st_params->gyro_limit & SELFTEST_GYRO_THRESH_MASK);
  tmp_stc_params |=
      (uint32_t)(st_params->avg_time & SELFTEST_AVERAGE_TIME_MASK);
  rc |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_STC_CONFIGPARAMS,
                                (uint8_t *)&tmp_stc_params);

  tmp_stc_params = 0; /* disable any selftest stop point */
  rc |=
      INV_IMU_WRITE_EDMP_SRAM(s, EDMP_STC_DEBUG_EN, (uint8_t *)&tmp_stc_params);
...
static int run_internal_selftest(inv_imu_device_t *s) {
  ...
  while (1) {
    ...
    rc |=
        inv_imu_read_reg(s, INT_APEX_STATUS1, 1, (uint8_t *)&int_apex_status1);
...
static int get_selftest_output(inv_imu_device_t *s,
  ...
  if (!st_params->accel_en) {
    ...
    st_output->accel_status =
        ((stc_results &
          (STC_RESULTS_ACCEL_X_MASK | STC_RESULTS_ACCEL_Y_MASK |
  ...
  if (!st_params->gyro_en) {
    ...
    st_output->gyro_status =
        ((stc_results &
          (STC_RESULTS_GYRO_X_MASK | STC_RESULTS_GYRO_Y_MASK |

 ...
 * It starts accelerometer in LP at 12.5Hz and will notify a WOM when any of the
 * 3 axis exceeds the threshold. The threshold can be configured through UART to
 * be either 200 mg or 24 mg and defaults to 200 mg
 */
  ...
  do {
    ...
    if (int1_flag) {
      ...
      if (int_state.INV_WOM_X || int_state.INV_WOM_Y || int_state.INV_WOM_Z) {
        INV_MSG(INV_MSG_LEVEL_INFO, "WoM at %10llu (X, Y, Z): %hhu, %hhu, %hhu",
                timestamp, int_state.INV_WOM_X, int_state.INV_WOM_Y,
                int_state.INV_WOM_Z);
      }
    }
...
static int configure_and_enable_wom(void) {
  ...
  /* Configure WOM to produce signal when at least one axis exceed
   * wom_threshold_high/low */
  rc |= inv_imu_adv_configure_wom(
      &imu_dev, wom_high_thr_en ? wom_threshold_high : wom_threshold_low,
...
static int get_uart_command() {
  ...
  case 'w':
    ...
    INV_MSG(INV_MSG_LEVEL_INFO, "WOM threshold %s.",
            wom_high_thr_en ? "High" : "Low");
    rc |= configure_and_enable_wom();
    SI_CHECK_RC(rc);
    break;
  case 'c':
...
static int print_help() {
  ...
  INV_MSG(INV_MSG_LEVEL_INFO, "#");
  INV_MSG(INV_MSG_LEVEL_INFO,
          "# 'w' : Toggle WOM threshold high (200mg) / low (24mg)");

...
/** < Defines the maximum number of characters that the uart driver will be able
 * to transmit in a single transfer */
...
#define UART_MNGR_TX_ABSOLUTE_MAX_TRANSFER_SIZE                                \
  ((INV_UART_SENSOR_CTRL_TX_MAX_TXFER_SIZE > INV_UART_LOG_TX_MAX_TXFER_SIZE)   \
       ? INV_UART_SENSOR_CTRL_TX_MAX_TXFER_SIZE                                \
       : INV_UART_LOG_TX_MAX_TXFER_SIZE)
 ...
 */
typedef struct {
  uint8_t *buffer; /** < Pointer to the start of the memory area allocated to
                     the module. Note: A part of this buffer will be yielded to
                     the UART driver while the rest will be used as a ring byte
                     buffer by the current module */
  uint16_t driver_max_single_txfer_size; /** < Size of the buffer yielded to the
                                            UART driver */
  uint16_t
      circ_buffer_size; /** < Size of the UART manager ring byte buffer
                          Note: total buffer size = driver_max_single_txfer_size
                          + circ_buffer_size */
  volatile uint16_t circ_buffer_head; /** < Pointer to the head of the UART
                                         manager ring byte buffer */
  volatile uint16_t circ_buffer_tail; /** < Pointer to the tail of the UART
                                         manager ring byte buffer */
} uart_mngr_buffer_struct_t;

/** @brief UART manager object definition.
 *
 * Contains all the fields needed by the current module in order to manage an
 * UART peripheral
 */
typedef struct {
...
static uart_mngr_struct_t um[2] = {

    {
        ...
        .uart_rx_buf = {.buffer = &sensor_ctrl_uart_rx_buffer[0],
                        .driver_max_single_txfer_size =
                            INV_UART_SENSOR_CTRL_RX_BUFFER_SIZE,
                        .circ_buffer_size =
                            0, /* whole RX buffer is reserved for uart driver */
                        .circ_buffer_head = 0,
                        .circ_buffer_tail = 0},
        .uart_tx_buf =
            {
                ...
                .driver_max_single_txfer_size =
                    INV_UART_SENSOR_CTRL_TX_MAX_TXFER_SIZE,
                .circ_buffer_size = INV_UART_SENSOR_CTRL_TX_BUFFER_SIZE -
                                    INV_UART_SENSOR_CTRL_TX_MAX_TXFER_SIZE,
                .circ_buffer_head = 0,
                .circ_buffer_tail = 0,
            },
        ...
        .uart_tx_buf =
            {
                ...
                .circ_buffer_size = INV_UART_LOG_TX_BUFFER_SIZE -
                                    INV_UART_LOG_TX_MAX_TXFER_SIZE,
                .circ_buffer_head = 0,
                .circ_buffer_tail = 0,
            },
 ...
 *  @param[out] data Pointer to the memory area where the popped data will be
 * stored
 *  @param[in]  len Number of bytes to be popped
 *  @note:      This function does not check if there are any bytes available
 * (waiting to be popped) in the ring byte buffer prior to popping them
 */
static void buffer_pop(uart_mngr_buffer_struct_t *uart_mngr, uint8_t *data,
                       uint16_t len) {
  ...
  if ((uart_mngr->circ_buffer_size - tail) >= len) {
    ...
    memcpy(&data[uart_mngr->circ_buffer_size - tail], &uart_mngr->buffer[start],
 ...
 */
static void buffer_push(uart_mngr_buffer_struct_t *uart_mngr, uint8_t *data,
                        uint16_t len) {
  /* start index of the circular buffer (first bytes are reserved for driver) */
  uint16_t start = uart_mngr->driver_max_single_txfer_size;
  ...
  if ((uart_mngr->circ_buffer_size - head) >= len) {
    ...
    memcpy(&uart_mngr->buffer[start], &data[uart_mngr->circ_buffer_size - head],
 ...
 *  @param[in]  uart_mngr Pointer to the UART manager buffer object
 *  @return     The number of bytes available (waiting to be popped) in the ring
 * byte buffer
 */
...
static void uart_tx_done_cb(void *context) {
  ...
  if (bytes_num) {
    if (bytes_num >= uart_mngr->uart_tx_buf.driver_max_single_txfer_size) {
      /* Clamp the number of bytes to be transfered to the maximum size
       * supported by the UART driver */
...
int inv_uart_mngr_init(inv_uart_mngr_init_struct_t *uart_mngr_init_struct) {
  ...
  /* Populate the driver init structure with data common for all the UART
   * peripherals */
  ...
  /* Populate the driver init structure with data specific to each UART
   * peripheral */
  uart_init_struct.tx_buffer = um[uart_nb].uart_tx_buf.buffer;
  uart_init_struct.rx_buffer = um[uart_nb].uart_rx_buf.buffer;
  uart_init_struct.tx_size =
      um[uart_nb].uart_tx_buf.driver_max_single_txfer_size;
  uart_init_struct.rx_size =
      um[uart_nb].uart_rx_buf.driver_max_single_txfer_size;
...
int inv_uart_mngr_puts(inv_uart_num_t uart, const char *s, unsigned short l) {
  ...
  if (uart_tx_state == INV_UART_STATE_IDLE) {
    if (l <= uart_mngr->uart_tx_buf.driver_max_single_txfer_size) {
       ...
       * make an UART transfer and push the rest of the data in the UART
       * manager's ring byte buffer
       */
      buffer_push(
          &uart_mngr->uart_tx_buf,
          (uint8_t *)&s[uart_mngr->uart_tx_buf.driver_max_single_txfer_size],
          l - uart_mngr->uart_tx_buf.driver_max_single_txfer_size);
      rc = inv_uart_puts(uart_mngr->uart_mngr_uart_num, s,
                         uart_mngr->uart_tx_buf.driver_max_single_txfer_size);
    }
    ...
    if (buffer_get_available(&uart_mngr->uart_tx_buf) < l) {
      ...
      /* Push the data in the ring byte buffer so that it can be transfered once
       * the current transfer finishes */
...
int inv_uart_mngr_getc(inv_uart_num_t uart) { return inv_uart_getc(uart); }

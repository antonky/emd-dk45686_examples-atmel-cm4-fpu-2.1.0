...
enum DynProSensorStatus {
  ...
  DYN_PRO_SENSOR_STATUS_STATE_CHANGED =
      1, /**< indicates a change in sensor state */
  DYN_PRO_SENSOR_STATUS_FLUSH_COMPLETE =
      2, /**< indicates the flush is completed */
  DYN_PRO_SENSOR_STATUS_POLLED_DATA =
      3, /**< sensor data value after manual request */
};
...
enum DynSensorType {
  ...
  DYN_PRO_SENSOR_TYPE_UNCAL_MAGNETOMETER =
  ...
  DYN_PRO_SENSOR_TYPE_GEOMAG_ROTATION_VECTOR =
  ...
  DYN_PRO_SENSOR_TYPE_BAC_EXTENDED =
      42, /**< Basic Activity Classifier Extended */
  DYN_PRO_SENSOR_TYPE_BAC_STATISTICS =
      43, /**< Basic Activity Classifier Statistics */
  ...
  DYN_PRO_SENSOR_TYPE_CUSTOM_BSCD, /**< Custom BAC StepCounter Calorie counter
  ...
  DYN_PRO_SENSOR_TYPE_OIS_1,    /**< Optical Image Stabilization with gyroscope
                                   only data instance 1 */
  ...
  DYN_PRO_SENSOR_TYPE_OIS_AG_0, /**< Optical Image Stabilization with
                                   accelerometer and gyroscope data instance 0
                                 */
  DYN_PRO_SENSOR_TYPE_OIS_AG_1, /**< Optical Image Stabilization with
                                   accelerometer and gyroscope data instance 1
                                 */
...
enum DynProtocolConfigType {
  ...
  DYN_PROTOCOL_CONFIG_TYPE_REFERENCE_FRAME =
      1, /**< sensor reference frame (mouting matrix) */
  DYN_PROTOCOL_CONFIG_TYPE_GAIN =
      2, /**< sensor gain to be applied on sensor data */
  DYN_PROTOCOL_CONFIG_TYPE_OFFSET =
      3, /**< sensor offset to be applied on sensor data */
  ...
  DYN_PROTOCOL_CONFIG_TYPE_POWER_MODE =
      9, /**< Change the power mode of the sensor */

  DYN_PROTOCOL_CONFIG_TYPE_CUSTOM =
      32, /**< base value to indicate custom config */

  DYN_PROTOCOL_CONFIG_TYPE_MAX =
      64, /**< absolute maximum value for config type */
};
 ...
 *
 *  Upon NEW_DATA event, event data are expected to point to a
...
typedef struct {
  ...
  uint32_t meta_data; /**< additionnal status/info regarding the sensor event:
                         (bit0-2: accuracy flag, bit3-31: reserved (must be
                         ignored) */
} DynProtocolSensorDataBase_t;
...
typedef struct {
  DynProtocolSensorDataBase_t
      base; /**< base dynamic protocol sensor data object */
  union {
    uint8_t u8[64];   /**< 64 bytes of data */
    uint16_t u16[32]; /**< 32 halfwords */
    uint32_t u32[16]; /**< 16 words */
  } data; /**< dynamic protocol sensor data (use a union to ensure correct
             pointer alignement) */
 ...
 */
typedef void (*DynProtocolEvent_cb)(enum DynProtocolEtype etype,

...
/*
 * This example showcases how to integrate and feed a 6 or 9 axis fusion
 * algorithm from IMU It retrieves AG sensor data at 400Hz from the FIFO using
 * the advanced driver. It allows to configure AG in either Low-Power or
 * Low-Noise mode, with the possibility to enable IMU high-resolution mode.
 * Magnetometer data can be retrieved from host MCU through its own I2C to feed
 * AGM fusion algorithm to output 9-axis quaternion. A sanity test can be
 * requested from UART to validate algorithm integration with test vector.
 */
...
static uint8_t
    fifo_data[FIFO_MIRRORING_SIZE]; /* Memory where to store FIFO data */
...
static uint8_t
    cal_save_en; /* Indicates if calibration shall be saved in flash */
 ...
 *  * Use a buffer for IMU as we have a FIFO which possibly contains multiple
 * packet
 ...
 */
#if defined(USE_EVB)
static int32_t mag_matrix[9] = {(1 << 30), 0, 0, 0,        (1 << 30),
                                0,         0, 0, (1 << 30)}; /* for EVB */
#elif defined(USE_DB)
static int32_t mag_matrix[9] = {-(1 << 30), 0, 0, 0,        -(1 << 30),
                                0,          0, 0, (1 << 30)}; /* for DB */
#else
static int32_t mag_matrix[9] = {0, (1 << 30), 0, -(1 << 30), 0,
                                0, 0,         0, (1 << 30)}; /* for DK */
#endif

/* Crystal on SmartMotion board is at 32768Hz, not 32000Hz, so is RTC/CLKIN
 * input to IMU, so if CLKIN is enabled, IMU sensor ODR will be shifted
 * accordingly as ODR is indicated for 32000Hz CLKIN input
 */
...
int main(void) {
  ...
  /* Setup message facility (log) and set callback to be called when a command
   * is received */
  ...
  do {
    ...
    if (int1_flag) {
      ...
      if (int_state.INV_FIFO_THS) {
        ...
        if (mag_init_successful) {
          ...
          if (imu_sample_count >= mag_acq_ths) {
            /*
             * The `imu_sample_count` should always be incremented by one,
             * except if the firmware was busy and several packets were pushed
             * in the FIFO (should not happen in mission mode). To get the
             * proper ODR in average, subtract the `mag_acq_ths` (the following
             * interrupt will occur slightly sooner to recover). If the
             * difference exceed `mag_acq_ths / 2`, don't try to recover and
             * reset the logic. This could result in a jitter on mag ODR up to
             * 50%.
             */
        ...
        /*
         * The FIFO threshold condition is re-evaluated after a FIFO read
         * operation. If an interrupt is asserted between FIFO count read and
         * FIFO data read, one packet will remain in the FIFO after the read
         * operation, therefore asserting an additional interrupt. Once we
         * parsed all packets, if we serviced all pending interrupt (`int1_flag`
         * is 0) and if a timestamp remains in the buffer, then we can safely
         * clear it as it doesn't match with a new sample but simply with a
       ...
       * In case the processing of the IMU data takes longer than 1 ODR, both
       * IMU and AKM interrupt flag might be set. Since IMU's ODR is faster or
       * equal to AKM's ODR, let's process IMU first by restarting the loop. AKM
...
static int setup_mcu() {
  ...
  /* When CLKIN is enabled, need to feed CLKIN pin with 32768Hz RTC from
   * SmartMotion */
  rc |= si_init_clkin();
#endif
...
static int setup_imu() {
...
#if INV_IMU_CLKIN_SUPPORTED
  /* CLKIN configuration */
  rc |= inv_imu_adv_set_int2_pin_usage(
      &imu_dev, IOC_PAD_SCENARIO_OVRD_INT2_CFG_OVRD_VAL_CLKIN);
  ...
  /* Set BW = ODR/4 */
  rc |= inv_imu_set_accel_ln_bw(&imu_dev,
                                IPREG_SYS2_REG_131_ACCEL_UI_LPFBW_DIV_4);
  rc |=
      inv_imu_set_gyro_ln_bw(&imu_dev, IPREG_SYS1_REG_172_GYRO_UI_LPFBW_DIV_4);
  SI_CHECK_RC(rc);
...
static int setup_mag() {
  ...
  if (whoami != AK0991x_COMPANY_ID) {
    INV_MSG(INV_MSG_LEVEL_ERROR,
            "Wrong WHOAMI for Mag device (read 0x%02x while expecting 0x%02x)",
            whoami, AK0991x_COMPANY_ID);
    rc |= INV_IMU_ERROR;
  }
  SI_CHECK_RC(rc);
...
static int init_agm_algo(void) {
  ...
  config.temp_sensitivity =
      (int32_t)((int64_t)((int64_t)1 << 30) / 128); // high-res;
...
static void sensor_event_cb(inv_imu_sensor_event_t *event) {
  ...
  /*
   * If no timestamp has been retrieved from interrupt handler (e.g. time_us ==
   * 0), then reconstruct the expected time using time from FIFO packet. This
   * can happen if the firmware is busy, for instance while writing biases into
   * flash.
   */
  if (time_us == 0) {
    uint64_t dt_fifo =
    ...
    INV_MSG(
        ...
        "No timestamp available in buffer. Using delta time from FIFO: %llu us",
        dt_fifo);
  }
...
static int save_calibration(void) {
   ...
   * no biases have been saved so far, or they are older than a predefined
   * period
   */
  if (cal_save_en && (last_save_us == 0 ||
                      current_us - last_save_us >= BIASES_SAVE_PERIOD_US)) {
    /*
     * Will be saved only if all accuracies are 3, ignoring mag if it wasn't
     * successfully initialized
     */
    int save_condition =
        (output.acc_accuracy_flag == 3) && (output.gyr_accuracy_flag == 3) &&
        ((mag_init_successful == 0) || (output.mag_accuracy_flag == 3));

    if (save_condition) {
      /*
       * With this configuration, the bias can be stored up to 186 iterations in
       * flash before erase sector. The erase procedure with the first write,
       * can take up to 250 ms. In this example, the erase is done dynamically
       * (when needed).
       */
      uint8_t data[84] = {0};
      uint8_t idx = 0;
...
static int print_current_biases() {
  INV_MSG(INV_MSG_LEVEL_INFO, "   - Accel: [%f %f %f] g",
          (float)acc_bias[0] / (1 << 16), (float)acc_bias[1] / (1 << 16),
          (float)acc_bias[2] / (1 << 16));

  INV_MSG(INV_MSG_LEVEL_INFO, "   - Gyro:  [%f %f %f]dps",
          (float)gyr_bias[0] / (1 << 16), (float)gyr_bias[1] / (1 << 16),
          (float)gyr_bias[2] / (1 << 16));

  INV_MSG(INV_MSG_LEVEL_INFO, "   - Mag:   [%f %f %f]uT",
          (float)mag_bias[0] / (1 << 16), (float)mag_bias[1] / (1 << 16),
          (float)mag_bias[2] / (1 << 16));

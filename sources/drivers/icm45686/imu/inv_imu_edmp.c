...
int inv_imu_edmp_set_frequency(inv_imu_device_t *s,
                               const dmp_ext_sen_odr_cfg_apex_odr_t frequency) {
  int status = INV_IMU_OK;
  dmp_ext_sen_odr_cfg_t dmp_ext_sen_odr_cfg;

  status |= inv_imu_read_reg(s, DMP_EXT_SEN_ODR_CFG, 1,
                             (uint8_t *)&dmp_ext_sen_odr_cfg);
  dmp_ext_sen_odr_cfg.apex_odr = frequency;
  status |= inv_imu_write_reg(s, DMP_EXT_SEN_ODR_CFG, 1,
                              (uint8_t *)&dmp_ext_sen_odr_cfg);
...
int inv_imu_edmp_init_apex(inv_imu_device_t *s) {
  ...
  /*
   * Initialize read and write pointers for pedometer to buffer full condition.
   * EDMP will always write step count in EDMP_PED_STEP_CNT_BUF2. Initialize
   * read and write pointers for freefall to 0.
   */
  status |=
      inv_imu_read_reg(s, APEX_BUFFER_MGMT, 1, (uint8_t *)&apex_buffer_mgmt);
  ...
  status |=
      inv_imu_write_reg(s, APEX_BUFFER_MGMT, 1, (uint8_t *)&apex_buffer_mgmt);

  /* Same impl as inv_imu_adv_power_up_sram, duplicated here to prevent
   * dependency */
  status |=
      inv_imu_read_reg(s, FIFO_SRAM_SLEEP, 1, (uint8_t *)&fifo_sram_sleep);
  fifo_sram_sleep.fifo_gsleep_shared_sram = 0x03;
  status |=
      inv_imu_write_reg(s, FIFO_SRAM_SLEEP, 1, (uint8_t *)&fifo_sram_sleep);
...
int inv_imu_edmp_recompute_apex_decimation(inv_imu_device_t *s) {
  ...
  /*
   * Check that DMP is turned OFF before requesting init APEX and save DMP
   * enabled bits before requesting init procedure
   */
  status |=
      inv_imu_read_reg(s, EDMP_APEX_EN0, 1, (uint8_t *)&save_edmp_apex_en0);
  status |=
      inv_imu_read_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&save_edmp_apex_en1);
  if (save_edmp_apex_en1.edmp_enable)
    return INV_IMU_ERROR;

  /*
   * Make sure that all DMP interrupts are masked by default, to not trigger
   * unexpected algorithm execution when initialization is done if any sensor is
   * running
   */
  value = 0x3F;
  ...
  status |= inv_imu_edmp_unmask_int_src(s, INV_IMU_EDMP_INT0,
                                        EDMP_INT_SRC_ON_DEMAND_MASK);

  /*
   * Request to execute init procedure, make sure init is the only feature
   * enabled (overwrite previously saved config)
   */
  ...
  /* Reset states */
  status |= inv_imu_edmp_mask_int_src(s, INV_IMU_EDMP_INT0,
                                      EDMP_INT_SRC_ON_DEMAND_MASK);

  /* Restore original DMP state, with DMP necessarily disabled as it was checked
   * at the beginning of this function */
  status |=
      inv_imu_write_reg(s, EDMP_APEX_EN0, 1, (uint8_t *)&save_edmp_apex_en0);
  status |=
      inv_imu_write_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&save_edmp_apex_en1);

  status |= inv_imu_edmp_unmask_int_src(s, INV_IMU_EDMP_INT0,
                                        EDMP_INT_SRC_ACCEL_DRDY_MASK);

  return status;
}

int inv_imu_edmp_get_apex_parameters(inv_imu_device_t *s,
  ...
  /* Pedometer */
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_PED_AMP_TH, (uint8_t *)&p->ped_amp_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_PED_STEP_CNT_TH,
                                   (uint8_t *)&p->ped_step_cnt_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_PED_STEP_DET_TH,
                                   (uint8_t *)&p->ped_step_det_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_PED_SB_TIMER_TH,
                                   (uint8_t *)&p->ped_sb_timer_th);
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_PED_HI_EN_TH, (uint8_t *)&p->ped_hi_en_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_PED_SENSITIVITY_MODE,
                                   (uint8_t *)&p->ped_sensitivity_mode);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_PED_LOW_EN_AMP_TH,
                                   (uint8_t *)&p->ped_low_en_amp_th);

  /* Tilt */
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_TILT_WAIT_TIME,
                                   (uint8_t *)&p->tilt_wait_time);

#if INV_IMU_USE_BASIC_SMD
  /* Basic SMD */
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_BASICSMD_WIN, (uint8_t *)&p->basicsmd_win);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_BASICSMD_WIN_WAIT,
                                   (uint8_t *)&p->basicsmd_win_wait);
#else
  /* SMD */
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_SMD_SENSITIVITY,
                                   (uint8_t *)&p->smd_sensitivity);
#endif
  ...
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_R2W_MOTION_PREV_GRAVITY_TIMEOUT,
                             (uint8_t *)&p->r2w_motion_prev_gravity_timeout);
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_R2W_LAST_GRAVITY_MOTION_TIMER,
  ...
  /* Freefall */
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_LOWG_PEAK_TH, (uint8_t *)&p->lowg_peak_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_LOWG_PEAK_TH_HYST,
                                   (uint8_t *)&p->lowg_peak_th_hyst);
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_LOWG_TIME_TH, (uint8_t *)&p->lowg_time_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_HIGHG_PEAK_TH,
                                   (uint8_t *)&p->highg_peak_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_HIGHG_PEAK_TH_HYST,
                                   (uint8_t *)&p->highg_peak_th_hyst);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_HIGHG_TIME_TH,
                                   (uint8_t *)&p->highg_time_th);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_FF_MIN_DURATION,
                                   (uint8_t *)&p->ff_min_duration);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_FF_MAX_DURATION,
                                   (uint8_t *)&p->ff_max_duration);
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_FF_DEBOUNCE_DURATION,
                                   (uint8_t *)&p->ff_debounce_duration);

  /* Tap */
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_TAP_MIN_JERK, (uint8_t *)&p->tap_min_jerk);
  ...
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_TAP_MAX_PEAK_TOL,
                                   (uint8_t *)&p->tap_max_peak_tol);
  ...
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_HARD_IRON_OFFSET,
                                   (uint8_t *)p->hard_iron_offset);

  /* Power save */
  status |= INV_IMU_READ_EDMP_SRAM(s, EDMP_POWER_SAVE_TIME,
                                   (uint8_t *)&p->power_save_time);
  status |= inv_imu_read_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&edmp_apex_en1);
  p->power_save_en =
      edmp_apex_en1.power_save_en ? INV_IMU_ENABLE : INV_IMU_DISABLE;

  return status;
}

int inv_imu_edmp_set_apex_parameters(inv_imu_device_t *s,
                                     const inv_imu_edmp_apex_parameters_t *p) {
  int status = INV_IMU_OK;
  edmp_apex_enx_t cfg;

  /* DMP cannot be configured if it is running, hence make sure all APEX
   * algorithms are off */
  status |= inv_imu_read_reg(s, EDMP_APEX_EN0, 2, (uint8_t *)&cfg);
  if (cfg.edmp_apex_en0.pedo_en || cfg.edmp_apex_en0.tilt_en ||
      cfg.edmp_apex_en0.ff_en || cfg.edmp_apex_en0.smd_en ||
      cfg.edmp_apex_en0.tap_en || cfg.edmp_apex_en0.r2w_en ||
      cfg.edmp_apex_en1.basic_smd_en ||
      cfg.edmp_apex_en1.soft_hard_iron_corr_en)
    return INV_IMU_ERROR;

  /* Pedometer */
  status |=
      INV_IMU_WRITE_EDMP_SRAM(s, EDMP_PED_AMP_TH, (uint8_t *)&p->ped_amp_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_PED_STEP_CNT_TH,
                                    (uint8_t *)&p->ped_step_cnt_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(
      s, EDMP_PED_PREV_STEP_CNT_TH,
      (uint8_t *)&p->ped_step_cnt_th); /* same as step_cnt_th */
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_PED_STEP_DET_TH,
                                    (uint8_t *)&p->ped_step_det_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_PED_SB_TIMER_TH,
                                    (uint8_t *)&p->ped_sb_timer_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_PED_HI_EN_TH,
                                    (uint8_t *)&p->ped_hi_en_th);
  ...
  /* Tilt */
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_TILT_WAIT_TIME,
                                    (uint8_t *)&p->tilt_wait_time);

#if INV_IMU_USE_BASIC_SMD
  /* Basic SMD */
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_BASICSMD_WIN,
                                    (uint8_t *)&p->basicsmd_win);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_BASICSMD_WIN_WAIT,
                                    (uint8_t *)&p->basicsmd_win_wait);
#else
  /* SMD */
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_SMD_SENSITIVITY,
                                    (uint8_t *)&p->smd_sensitivity);
#endif
  ...
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_R2W_MOTION_THR_TIMER_SLOW,
                                    (uint8_t *)&p->r2w_motion_th_timer_slow);
  status |=
      INV_IMU_WRITE_EDMP_SRAM(s, EDMP_R2W_MOTION_PREV_GRAVITY_TIMEOUT,
                              (uint8_t *)&p->r2w_motion_prev_gravity_timeout);
  status |=
      INV_IMU_WRITE_EDMP_SRAM(s, EDMP_R2W_LAST_GRAVITY_MOTION_TIMER,
                              (uint8_t *)&p->r2w_last_gravity_motion_timer);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_R2W_LAST_GRAVITY_TIMEOUT,
                                    (uint8_t *)&p->r2w_last_gravity_timeout);
  status |=
      INV_IMU_WRITE_EDMP_SRAM(s, EDMP_R2W_GESTURE_VALIDITY_TIMEOUT,
                              (uint8_t *)&p->r2w_gesture_validity_timeout);

  /* Free Fall */
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_LOWG_PEAK_TH,
                                    (uint8_t *)&p->lowg_peak_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_LOWG_PEAK_TH_HYST,
                                    (uint8_t *)&p->lowg_peak_th_hyst);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_LOWG_TIME_TH,
                                    (uint8_t *)&p->lowg_time_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_HIGHG_PEAK_TH,
                                    (uint8_t *)&p->highg_peak_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_HIGHG_PEAK_TH_HYST,
                                    (uint8_t *)&p->highg_peak_th_hyst);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_HIGHG_TIME_TH,
                                    (uint8_t *)&p->highg_time_th);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_FF_MIN_DURATION,
                                    (uint8_t *)&p->ff_min_duration);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_FF_MAX_DURATION,
                                    (uint8_t *)&p->ff_max_duration);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_FF_DEBOUNCE_DURATION,
                                    (uint8_t *)&p->ff_debounce_duration);

  /* Tap */
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_TAP_MIN_JERK,
                                    (uint8_t *)&p->tap_min_jerk);
  ...
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_TAP_MAX_PEAK_TOL,
                                    (uint8_t *)&p->tap_max_peak_tol);
  ...
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_SOFT_IRON_SENSITIVITY_MATRIX,
                                    (uint8_t *)p->soft_iron_sensitivity_matrix);
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_HARD_IRON_OFFSET,
                                    (uint8_t *)p->hard_iron_offset);

  /* Power save */
  status |= INV_IMU_WRITE_EDMP_SRAM(s, EDMP_POWER_SAVE_TIME,
                                    (uint8_t *)&p->power_save_time);
  status |=
      inv_imu_read_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&cfg.edmp_apex_en1);
  cfg.edmp_apex_en1.power_save_en = p->power_save_en;
  status |=
      inv_imu_write_reg(s, EDMP_APEX_EN1, 1, (uint8_t *)&cfg.edmp_apex_en1);

  return status;
}

int inv_imu_edmp_get_config_int_apex(inv_imu_device_t *s,
  ...
  it->INV_STEP_CNT_OVFL =
      !cfg.int_apex_config0.int_status_mask_pin_step_cnt_ovfl;
...
int inv_imu_edmp_set_config_int_apex(inv_imu_device_t *s,
...
/*
 * Check if inv_imu_edmp_set_frequency() was called without recomputing APEX
 * decimation thanks to inv_imu_edmp_recompute_apex_decimation(). This function
 * will compare edmp decimation rate in IMU SRAM as computed at APEX init step
 * vs the DMP ODR currently written in IMU register. Returns
 * INV_IMU_ERROR_EDMP_ODR if inv_imu_edmp_recompute_apex_decimation() should
 * have been called.
 */
static int check_dmp_odr_decimation(inv_imu_device_t *s) {
  int status = INV_IMU_OK;
  ...
  status |=
      inv_imu_read_sram(s, 0x460 /* edmp decimation rate address in SRAM */, 1,
...
int inv_imu_edmp_get_int_apex_status(inv_imu_device_t *s,
                                     inv_imu_edmp_int_state_t *it) {
  int status = INV_IMU_OK;
...
int inv_imu_edmp_get_pedometer_data(inv_imu_device_t *s,
  ...
  /*
   * Read value multiple times in case the buffer was written while we were
   * reading it. If we read the same value twice consecutively, it means we have
   * a proper value.
   */
...
int inv_imu_edmp_get_tap_data(inv_imu_device_t *s,
  ...
  status |=
      INV_IMU_READ_EDMP_SRAM(s, EDMP_TAP_DIR, (uint8_t *)&(data->direction));
...
int inv_imu_edmp_mask_int_src(inv_imu_device_t *s,
...
int inv_imu_edmp_unmask_int_src(inv_imu_device_t *s,
                                inv_imu_edmp_int_t edmp_int_nb,
                                uint8_t int_mask) {
  int status = INV_IMU_OK;
  uint32_t reg_addr;
...
int inv_imu_edmp_run_ondemand(inv_imu_device_t *s,
                              inv_imu_edmp_int_t edmp_int_nb) {
  int status = INV_IMU_OK;
  reg_host_msg_t reg_host_msg;

  status |=
      inv_imu_edmp_unmask_int_src(s, edmp_int_nb, EDMP_INT_SRC_ON_DEMAND_MASK);
...
int inv_imu_edmp_wait_for_idle(inv_imu_device_t *s) {
  ...
  /* Wait for idle == 1 (indicates EDMP is not running, e.g execution is
   * completed) */
  while (status == INV_IMU_OK) {

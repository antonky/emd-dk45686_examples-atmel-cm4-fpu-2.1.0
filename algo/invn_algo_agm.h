 ...
 *  @warning Supported gyroscope FSR [250 dps, 500 dps, 1000 dps, 2000 dps, 4000
 * dps]
...
#define INVN_ALGO_AGM_DATA_STRUCTURE_SIZE                                      \
  2150 /**< Size of the algo data structure */
 ...
 */
#define INVN_ALGO_AGM_OUTPUT_MASK_ACCEL_CAL                                    \
  (1 << 0) /**< Cal Accel update mask */
...
#define INVN_ALGO_AGM_OUTPUT_MASK_QUAT_AG                                      \
  (1 << 3) /**< AG fusion update mask (GRV: Game Rotation Vector) */
#define INVN_ALGO_AGM_OUTPUT_MASK_QUAT_AM                                      \
  (1 << 4) /**< AM fusion update mask (GeomagRV: Geomagnetic Rotation Vector)  \
            */
#define INVN_ALGO_AGM_OUTPUT_MASK_QUAT_AGM                                     \
  (1 << 5) /**< AGM fusion update mask (RV: Rotation Vector) */
#define INVN_ALGO_AGM_OUTPUT_MASK_GRAVITY (1 << 6) /**< Gravity update mask */
#define INVN_ALGO_AGM_OUTPUT_MASK_LINEARACC                                    \
  (1 << 7) /**< Linear acceleration update mask */
...
typedef struct {
  ...
  int64_t sRimu_time_us; /**< Accel and Gyro timestamp in us. Use 0 if not
                            available. */
  int32_t
      sRacc_data[3]; /**< Raw accel coded over 20 bits (FSR g = 1 << 19 LSB) */
  int32_t
      sRgyr_data[3]; /**< Raw gyro coded over 20 bits (FSR dps = 1 << 19 LSB) */
...
typedef struct {
  ...
  int8_t acc_accuracy_flag; /**< Accel accuracy from 0 (not calibrated) to 3
                               (well calibrated) */
  ...
  int8_t gyr_accuracy_flag; /**< Gyro accuracy from 0 (not calibrated) to 3
                               (well calibrated) */
  ...
  int8_t mag_accuracy_flag; /**< Mag accuracy from 0 (not calibrated) to 3 (well
                               calibrated) */

  int32_t grv_quat_q30[4]; /**< GRV quaternion (Accel and Gyro fusion) with WXYZ
                              convention coded as Q30 */

  int32_t gmrv_quat_q30[4]; /**< GeomagRV quaternion (Accel and Mag fusion) with
                               WXYZ convention coded as Q30 */
  int32_t gmrv_accuracy_q27; /**< GeomagRV heading accuracy estimation (1-sigma)
                                in rad coded as Q27 */
  int32_t gmrv_accuracy_3sigma_q27; /**< GeomagRV heading accuracy estimation
                                       (3-sigma) in rad coded as Q27 */

  int32_t rv_quat_q30[4];  /**< RV quaternion (Accel, Gyro and Mag fusion) with
                              WZYZ convention coded as Q30 */
  int32_t rv_accuracy_q27; /**< RV heading accuracy estimation (1-sigma) in rad
                              coded as Q27 */
  int32_t rv_accuracy_3sigma_q27; /**< RV heading accuracy estimation (3-sigma)
                                     in rad coded as Q27 */

  int32_t gravity_q16[3]; /**< Gravity estimation in sensor frame */
  int32_t
      linear_acc_q16[3]; /**< Linear acceleration estimation in sensor frame */
...
typedef struct {
  int32_t *acc_bias_q16; /**< Initial Accel biases coded as Q16 (will use
                            {0,0,0} if pointer is NULL) */
  int32_t *gyr_bias_q16; /**< Initial Gyro biases coded as Q16 (will use {0,0,0}
                            if pointer is NULL) */
  int32_t *mag_bias_q16; /**< Initial Mag biases coded as Q16 (will use {0,0,0}
                            if pointer is NULL) */
  ...
  int32_t temp_sensitivity; /**< Temperature sensitivity in Celcius/LSB coded as
                               Q30 */
  int32_t
      temp_offset; /**<  Temperature offset in degrees Celcius coded as Q16 */

  int32_t gyr_cal_stationary_duration_us; /**< Duration in us for no motion
                                             window used for gyro bias
                                             calibration. Range: [0, 6000000] */
  int32_t gyr_cal_sample_number_log2; /**< Define the number of gyro samples (N
                                         = 2^gyr_cal_sample_number_log2) used
                                         for calibration. Range: [6, 8] */
  int32_t gyr_cal_threshold_metric1;  /**< Stationary detection threshold of 1st
                                         metric for gyro calibration. Range [0,
                                         1200] */
  int32_t gyr_cal_threshold_metric2;  /**< Stationary detection threshold of 2nd
                                         metric for gyro calibration. Range [0,
                                         80000] */
} InvnAlgoAGMConfig;
 ...
 */
int invn_algo_agm_generate_config(InvnAlgoAGMConfig *config,
                                  InvnAlgoAGMModeId mode_id);

/** @brief Initializes algo according to config structure provided and reset
 * internal states.
 *  @param[in] self    Pointer to algo structure.
 *  @param[in] config  Desired configuration.
 *  @return            0 on success, negative value on error.
 */
int invn_algo_agm_init(InvnAlgoAGMStruct *self,
                       const InvnAlgoAGMConfig *config);
 ...
 *  @return            0 on success, negative value on error.
 */
int invn_algo_agm_set_config(InvnAlgoAGMStruct *self,
                             const InvnAlgoAGMConfig *config);
 ...
 *  @return                  0 on success, negative value on error.
 */
int invn_algo_agm_enable_calibration(InvnAlgoAGMStruct *self,
                                     const int8_t gyro_cal_en,
                                     const int8_t accel_cal_en,
                                     const int8_t mag_cal_en);
 ...
 *  @return             0 on success, negative value on error.
 */
int invn_algo_agm_process(InvnAlgoAGMStruct *self,
                          const InvnAlgoAGMInput *input,

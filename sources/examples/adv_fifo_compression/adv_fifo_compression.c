...
/*
 * This example showcases how to retrieve data from the FIFO while using FIFO
 * compression. It enables accelerometer and/or gyroscope at 50Hz and
 * enables/disable compression upon user request through UART. FIFO compression
 * is enabled by default, with a watermark configuration of 16 samples and a
...
#define SLIDING_AVERAGE(AVG, N, DATA)                                          \
  (float)((AVG * (double)(N - 1) + DATA) / (double)N)
...
int main(void) {
  ...
  INV_MSG(INV_MSG_LEVEL_INFO,
          "### Example FIFO Compression (using advanced API)");
  INV_MSG(INV_MSG_LEVEL_INFO, "###");
  ...
  do {
    ...
    if (int1_flag) {
      ...
      if (int_state.INV_FIFO_THS) {
        ...
        INV_MSG(INV_MSG_LEVEL_INFO,
                "SI %10llu us   FIFO count %u   Sample count %u", timestamp,
                (uint32_t)fifo_count, MAX(acc_stats.count, gyr_stats.count));

        if (accel_en)
          snprintf(accel_str, 60, "Average Accel:% 8.2f % 8.2f % 8.2f g",
                   acc_stats.average[0], acc_stats.average[1],
                   acc_stats.average[2]);
        else
        ...
        if (gyro_en)
          snprintf(gyro_str, 60, "Average Gyro:% 8.2f % 8.2f % 8.2f dps",
                   gyr_stats.average[0], gyr_stats.average[1],
                   gyr_stats.average[2]);
        else
        ...
        if (compression_en && !(accel_en && gyro_en))
          ...
          snprintf(temp_str, 40, "Average Temp: % 4.2f degC",
                   temp_stats.average[0]);
...
static int configure_fifo(void) {
   ...
   * Non-compressed frame every 8 frames
   */
  fifo_config.base_conf.fifo_wm_th =
      16; // watermark set to 16 frames = 256 bytes
...
static void sensor_event_cb(inv_imu_sensor_event_t *event) {
  /*
   * Accel and gyro data are coded as 16-bits signed (max_lsb = 2^(16-1) =
   * 32768) with the configured FSR (4 g and 2000 dps, see `setup_imu()`
   * function). Temperature is coded as 8-bits signed with a scale factor of 2
   * and an offset of 25 °C.
   */
  int max_lsb = 32768;
  ...
  if (event->sensor_mask & (1 << INV_SENSOR_ACCEL)) {
    ...
    acc_stats.average[0] =
        SLIDING_AVERAGE(acc_stats.average[0], acc_stats.count, accel_g[0]);
    acc_stats.average[1] =
        SLIDING_AVERAGE(acc_stats.average[1], acc_stats.count, accel_g[1]);
    acc_stats.average[2] =
        SLIDING_AVERAGE(acc_stats.average[2], acc_stats.count, accel_g[2]);
  }
  ...
  if (event->sensor_mask & (1 << INV_SENSOR_GYRO)) {
    ...
    gyr_stats.average[0] =
        SLIDING_AVERAGE(gyr_stats.average[0], gyr_stats.count, gyro_dps[0]);
    gyr_stats.average[1] =
        SLIDING_AVERAGE(gyr_stats.average[1], gyr_stats.count, gyro_dps[1]);
    gyr_stats.average[2] =
        SLIDING_AVERAGE(gyr_stats.average[2], gyr_stats.count, gyro_dps[2]);
  }
  ...
  if (event->sensor_mask & (1 << INV_SENSOR_TEMPERATURE)) {
    ...
    temp_stats.count++;
    temp_stats.average[0] =
        SLIDING_AVERAGE(temp_stats.average[0], temp_stats.count, temp_degc);
  }
}
...
static int print_current_config() {
  ...
  INV_MSG(INV_MSG_LEVEL_INFO, "# FIFO compression: %s",
          compression_en ? "Enabled" : "Disabled");
  INV_MSG(INV_MSG_LEVEL_INFO, "#");

...
/*
 * Used to artificially increase resolution of gyro and mag bias through UCAL sensors.
 * Default to 1, can be changed with sensorcli (`setconfig gyr 32=mult`). 
 * See `handle_command` function for details.
 */
 ...
 */
static void transport_event_cb(enum DynProTransportEvent e, union DynProTransportEventData data,
                               void *cookie);
static void protocol_event_cb(enum DynProtocolEtype etype, enum DynProtocolEid eid,
                              const DynProtocolEdata_t *edata, void *cookie);
static int  handle_command(enum DynProtocolEid eid, const DynProtocolEdata_t *edata,
                 ...
                 const InvnAlgoAGMOutput *output)
{
	...
	if (self.racc_en && (input->mask & INVN_ALGO_AGM_INPUT_MASK_ACC)) {
		...
		dp_data.d.async.sensorEvent.vdata.data.u32[0] = (uint32_t)input->sRacc_data[0];
		dp_data.d.async.sensorEvent.vdata.data.u32[1] = (uint32_t)input->sRacc_data[1];
		dp_data.d.async.sensorEvent.vdata.data.u32[2] = (uint32_t)input->sRacc_data[2];
	...
	if (self.rgyr_en && (input->mask & INVN_ALGO_AGM_INPUT_MASK_GYR)) {
		...
		dp_data.d.async.sensorEvent.vdata.data.u32[0] = (uint32_t)input->sRgyr_data[0];
		dp_data.d.async.sensorEvent.vdata.data.u32[1] = (uint32_t)input->sRgyr_data[1];
		dp_data.d.async.sensorEvent.vdata.data.u32[2] = (uint32_t)input->sRgyr_data[2];
		send_dp_data(&dp_data);
	}

	if (self.rtemp_en && (input->mask & INVN_ALGO_AGM_INPUT_MASK_ACC)) {
		dp_data.sensor_id                             = DYN_PRO_SENSOR_TYPE_RAW_TEMPERATURE;
		dp_data.d.async.sensorEvent.vdata.data.u32[0] = (uint32_t)input->sRtemp_data;
		send_dp_data(&dp_data);
	}

	if (self.rmag_en && (input->mask & INVN_ALGO_AGM_INPUT_MASK_MAG)) {
		dp_data.sensor_id                             = DYN_PRO_SENSOR_TYPE_RAW_MAGNETOMETER;
		dp_data.d.async.sensorEvent.vdata.data.u32[0] = (uint32_t)input->sRmag_data[0];
		dp_data.d.async.sensorEvent.vdata.data.u32[1] = (uint32_t)input->sRmag_data[1];
		dp_data.d.async.sensorEvent.vdata.data.u32[2] = (uint32_t)input->sRmag_data[2];
		send_dp_data(&dp_data);
	}
	...
	if (self.temp_en && (output->mask & INVN_ALGO_AGM_OUTPUT_MASK_ACCEL_CAL)) {
		...
		dp_data.d.async.sensorEvent.vdata.base.meta_data = 0; // Accuracy flag not available
	...
	if (self.rv_en && (output->mask & INVN_ALGO_AGM_OUTPUT_MASK_QUAT_AGM)) {
		...
		dp_data.d.async.sensorEvent.vdata.data.u32[4]    = output->rv_accuracy_q27 >> 11; /* Q16 */
		dp_data.d.async.sensorEvent.vdata.base.meta_data = 0;
...
int check_received_command()
{
	int rc = 0;
	/*
	 * Look for RX bytes and feed Dynamic protocol transport (will ultimately call Dynamic protocol event handler)
	 */
                               ...
                               void *cookie)
{
	...
	case DYN_PRO_TRANSPORT_EVENT_ERROR:
		INV_MSG(INV_MSG_LEVEL_ERROR,
		        "ERROR event with value %d received from Dynamic Protocol transport", data.error);
		break;
	...
	case DYN_PRO_TRANSPORT_EVENT_PKT_BYTE:
		...
		if (rc < 0)
			INV_MSG(INV_MSG_LEVEL_DEBUG, "DynProtocol_processPktByte(%02x) returned %d",
			        data.pkt_byte, rc);
		break;
                              ...
                              const DynProtocolEdata_t *edata, void *cookie)
{
	...
	case DYN_PROTOCOL_ETYPE_CMD:
		...
		if (DynProtocol_encodeResponse(&protocol, eid, &resp_edata, uart_frame.payload_data,
		...
		if ((rc = inv_uart_mngr_puts(SI_UART_ID_FTDI, (const char *)uart_frame.header,
			...
			INV_MSG(INV_MSG_LEVEL_DEBUG,
			        "emdwrapper_protocol_event_cb: 'uart_mngr_puts()' returned %d", rc);
	...
	default:
		INV_MSG(INV_MSG_LEVEL_WARNING, "FEUartDynProt: unexpected packet received. Ignored.");
                          ...
                          DynProtocolEdata_t *respdata)
{
	...
	case DYN_PROTOCOL_EID_GET_SW_REG:
		...
		if (edata->d.command.regAddr == DYN_PROTOCOL_EREG_HANDSHAKE_SUPPORT)
			return (inv_uart_get_flow_control_configuration(SI_UART_ID_FTDI) == 0) ? 0 : 1;
		else
			return 0;
	...
	case DYN_PROTOCOL_EID_PING_SENSOR:
		...
		case DYN_PRO_SENSOR_TYPE_RAW_MAGNETOMETER:
			return command_cb(IS_MAG_INIT_SUCCESSFUL) ? INV_ERROR_SUCCESS : INV_ERROR_BAD_ARG;
		default:
			return INV_ERROR_BAD_ARG;
		}
	...
	case DYN_PROTOCOL_EID_SET_SENSOR_CFG:
		...
		case DYN_PROTOCOL_CONFIG_TYPE_CUSTOM + 0: {
			...
			INV_MSG(INV_MSG_LEVEL_INFO,
			        "Setting multiplier factor for ugyr and umag to %u (max biases is %u dps/uT)",
			        biases_multiplier,
			        2048 /* max value that can be coded in dynprot */ / biases_multiplier);
			return INV_ERROR_SUCCESS;
		}
...
static int send_dp_data(DynProtocolEdata_t *dp_data)
{
	...
	rc = DynProtocol_encodeAsync(&protocol, DYN_PROTOCOL_EID_NEW_SENSOR_DATA, dp_data,
	                             uart_frame.payload_data, uart_frame.max_payload_len,
	...
	if ((rc = inv_uart_mngr_puts(SI_UART_ID_FTDI, (const char *)uart_frame.header,
	                             (unsigned short)uart_frame.len)) != 0)
		INV_MSG(INV_MSG_LEVEL_DEBUG, "sensor_event_cb: 'uart_mngr_puts()' returned %d", rc);

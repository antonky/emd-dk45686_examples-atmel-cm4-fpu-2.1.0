...
static struct gpio_mapping gm[INV_GPIO_BOARD_MAX][INV_GPIO_MAX] = {
    ...
    {{// GPIO_INV_SENSOR_INT1
     ...
     {// GPIO_SW0_BUTTON
      ...
      .pio_type = PIO_INPUT,
      .pio_attributes =
          (PIO_DEFAULT | PIO_PULLUP | PIO_DEBOUNCE | PIO_IT_FALL_EDGE),
      .pio_irqn = PIOA_IRQn,
      .callback = 0,
      .context = 0},
     {// INV_GPIO_CLKIN
...
void inv_gpio_init_pin_out(unsigned pin_num) {
  ...
  pio_set_output(gm[invn_gpio_board_rev][pin_num].p_pio,
                 gm[invn_gpio_board_rev][pin_num].pio_mask, LOW, DISABLE,
                 DISABLE);
}

void inv_gpio_init_pin_input(unsigned pin_num) {
  pmc_enable_periph_clk(gm[invn_gpio_board_rev][pin_num].pio_id);
  pio_set_input(gm[invn_gpio_board_rev][pin_num].p_pio,
                gm[invn_gpio_board_rev][pin_num].pio_mask, PIO_DEFAULT);
}

void inv_gpio_toggle_pin(unsigned pin_num) {
  pio_toggle_pin_group(gm[invn_gpio_board_rev][pin_num].p_pio,
                       gm[invn_gpio_board_rev][pin_num].pio_mask);
}

void inv_gpio_set_pin_high(unsigned pin_num) {
  pio_set_pin_group_high(gm[invn_gpio_board_rev][pin_num].p_pio,
                         gm[invn_gpio_board_rev][pin_num].pio_mask);
}

void inv_gpio_set_pin_low(unsigned pin_num) {
  pio_set_pin_group_low(gm[invn_gpio_board_rev][pin_num].p_pio,
                        gm[invn_gpio_board_rev][pin_num].pio_mask);
}
...
int inv_gpio_get_status(unsigned pin_num) {
  ...
  return (int)pio_get(gm[invn_gpio_board_rev][pin_num].p_pio,
                      gm[invn_gpio_board_rev][pin_num].pio_type,
                      gm[invn_gpio_board_rev][pin_num].pio_mask);
}
...
int inv_gpio_output_clk_on_pin(unsigned pin_num) {
  ...
  if (gm[invn_gpio_board_rev][pin_num].pio_index == PIO_PA17_IDX) {
    ...
    ioport_set_pin_mode(gm[invn_gpio_board_rev][pin_num].pio_index,
                        IOPORT_MODE_MUX_B);
...
void inv_gpio_sensor_irq_init(unsigned pin_num,
  ...
  pio_configure(gm[invn_gpio_board_rev][pin_num].p_pio,
                gm[invn_gpio_board_rev][pin_num].pio_type,
                gm[invn_gpio_board_rev][pin_num].pio_mask,
                gm[invn_gpio_board_rev][pin_num].pio_attributes);

  pio_handler_set(gm[invn_gpio_board_rev][pin_num].p_pio,
                  gm[invn_gpio_board_rev][pin_num].pio_id,
                  gm[invn_gpio_board_rev][pin_num].pio_mask,
                  gm[invn_gpio_board_rev][pin_num].pio_attributes,
                  gpio_common_callback);

  pio_enable_interrupt(gm[invn_gpio_board_rev][pin_num].p_pio,
                       gm[invn_gpio_board_rev][pin_num].pio_mask);

  pio_handler_set_priority(gm[invn_gpio_board_rev][pin_num].p_pio,
                           gm[invn_gpio_board_rev][pin_num].pio_irqn,
                           0); // TODO: make the priority configurable
}

static void gpio_common_callback(const uint32_t id, const uint32_t index) {
  for (unsigned i = 0; i < sizeof(gm) / sizeof(gm[invn_gpio_board_rev][0]);
       i++) {
    if ((gm[invn_gpio_board_rev][i].pio_id == id) &&
        (gm[invn_gpio_board_rev][i].pio_mask == index)) {
      if (gm[invn_gpio_board_rev][i].callback)
        gm[invn_gpio_board_rev][i].callback(gm[invn_gpio_board_rev][i].context,
                                            i);
      break;
    }
  }

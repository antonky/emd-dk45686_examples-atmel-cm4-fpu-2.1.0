...
/*
 * This example showcases how to retrieve data from the FIFO using the basic
 * driver. It prints accelerometer and/or gyroscope data in either SI unit or in
 * raw data format. High-resolution mode can be enabled and sensors
 * Low-Power/Low-Noise can be configured. It starts accelerometer and gyroscope
 * in Low-Noise mode at 50Hz by default.
 */
...
static uint8_t discard_accel_samples; /* Indicates how many accel samples should
                                         be discarded */
static uint8_t discard_gyro_samples;  /* Indicates how many gyro samples should
                                         be discarded */
    ...
    if (int1_flag) {
      ...
      if (int_state.INV_FIFO_THS) {
        ...
        for (uint16_t i = 0; i < frame_count; i++) {
          ...
          float temp_degc = 0;
          /*
           * In hires mode, accel and gyro data are coded as 20-bits signed
           * (max_lsb = 2^(20-1) = 524288) with max FSR (32 g and 4000 dps).
           * Temperature is coded as 16-bits signed with a scale factor of 128
           * and an offset of 25 °C. Otherwise, accel and gyro data are coded as
           * 16-bits signed (max_lsb = 2^(16-1) * = 32768) with the configured
           * FSR (4 g and 2000 dps, see `setup_imu()` function). Temperature is
           * coded as 8-bits signed with a scale factor of 2 and an offset of 25
           * °C.
           */
          ...
          if (hires_en) {
            ...
            if ((discard_accel_samples == 0) &&
                (accel_raw[0] != INVALID_VALUE_FIFO) &&
          ...
          temp_degc =
              (float)temp_offset_c + ((float)temp_raw / temp_scale_factor);
          ...
          if (print_si) {
            if (is_accel_data_valid)
              snprintf(accel_str, 40, "Accel:% 8.2f % 8.2f % 8.2f g",
                       accel_g[0], accel_g[1], accel_g[2]);
            else
              snprintf(accel_str, 40, "Accel:       -        -        -  ");

            if (is_gyro_data_valid)
              snprintf(gyro_str, 40, "Gyro:% 8.2f % 8.2f % 8.2f dps",
            ...
            INV_MSG(INV_MSG_LEVEL_INFO, "SI  %10llu us   %s   %s   %s   %s",
          ...
          if (print_lsb) {
            ...
            if (is_gyro_data_valid)
              snprintf(gyro_str, 40, "Gyro:% 8d % 8d % 8d", gyro_raw[0],
            ...
            INV_MSG(INV_MSG_LEVEL_INFO,
                    "LSB %10llu us   %s     %s       %s        %s", timestamp,
...
static int get_uart_command() {
  ...
  case 's':
    print_si = !print_si;
    INV_MSG(INV_MSG_LEVEL_INFO, "%s SI print.",
            print_si ? "Enabling" : "Disabling");
    break;
  case 'l':
    print_lsb = !print_lsb;
    INV_MSG(INV_MSG_LEVEL_INFO, "%s LSB print.",
            print_lsb ? "Enabling" : "Disabling");
    break;
  case 'a':
    rc |= configure_accel(!accel_en);
    accel_en = !accel_en;
    INV_MSG(INV_MSG_LEVEL_INFO, "%s Accel.",
            accel_en ? "Enabling" : "Disabling");
    break;
  case 'g':
  ...
  case 'i':
    hires_en = !hires_en;
    INV_MSG(INV_MSG_LEVEL_INFO, "%s high-res mode.",
            hires_en ? "Enabling" : "Disabling");
    rc |= configure_hires(hires_en);
    break;
  case 'p':
    use_ln = !use_ln;
    INV_MSG(INV_MSG_LEVEL_INFO, "%s selected.",
            use_ln ? "Low-noise" : "Low-power");
    rc |= configure_accel(accel_en);
    rc |= configure_gyro(gyro_en);
    break;
  case 'c':
...
static int print_help() {
  ...
  INV_MSG(INV_MSG_LEVEL_INFO,
          "# 'p' : Toggle power mode (low-noise, low-power)");
...
static int print_current_config() {
  ...
  INV_MSG(INV_MSG_LEVEL_INFO, "# High-resolution mode: %s",
          hires_en ? "Enabled" : "Disabled");
  INV_MSG(INV_MSG_LEVEL_INFO, "# Power mode: %s",
          use_ln ? "Low-noise" : "Low-power");
  INV_MSG(INV_MSG_LEVEL_INFO, "#");
  INV_MSG(INV_MSG_LEVEL_INFO, "# Data in SI are %s",
          print_si ? "printed" : "hidden");
  INV_MSG(INV_MSG_LEVEL_INFO, "# Data in LSB are %s",
          print_lsb ? "printed" : "hidden");
  INV_MSG(INV_MSG_LEVEL_INFO, "#");

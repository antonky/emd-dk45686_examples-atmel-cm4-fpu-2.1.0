...
/** TX/RX frames have the following format: 0x55 0xAA <NB_BYTES (2)> <PACKET
 * (n)> */
...
static inline void
DynProTransportUart_callEventCB(DynProTransportUart_t *self,
...
/** @brief This function forces protocol to use DMA when transfering data on
 * UART
 *
 ...
 *  @note : This function should be called after DynProTransportUart_init
 * function. It was not merged with DynProTransportUart_init() to avoid any API
 * break.
 *
...
int DynProTransportUart_checkHeader_fromISR(uint8_t *rcv_byte) {
  ...
  if (expected_size >
      128) // Arbitrary reasonable value to allow very quick check
    return -1;
  else
    return expected_size;
}

int DynProTransportUart_rxProcessByte(DynProTransportUart_t *self,
                                      uint8_t rcv_byte) {
  ...
  case RECEIVER_STATE_IDLE:
    if (rcv_byte == SYNC_BYTE_0) {
      ...
      INV_MSG(INV_MSG_LEVEL_VERBOSE,
              "DynProTransportUart: unexpected SYNC0 byte %x recevied",
              rcv_byte);
      self->rx_sm_state = RECEIVER_STATE_IDLE;
      udata.error = -1;
      DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_ERROR,
                                      udata);
      return -1;
    }
    break;
  ...
  case RECEIVER_STATE_SYNC_1:
    if (rcv_byte == SYNC_BYTE_1) {
      ...
      INV_MSG(INV_MSG_LEVEL_VERBOSE,
              "DynProTransportUart: unexpected SYNC1 byte %x recevied",
              rcv_byte);
      self->rx_sm_state = RECEIVER_STATE_IDLE;
      udata.error = -1;
      DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_ERROR,
  ...
  case RECEIVER_STATE_SIZE_BYTE_1:
    ...
    DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_PKT_SIZE,
                                    udata);
    return 1;
  ...
  case RECEIVER_STATE_PACKET_DATA:
    ...
    DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_PKT_BYTE,
                                    udata);
    if (self->rx_received_bytes == self->rx_expected_bytes) {
      self->rx_sm_state = RECEIVER_STATE_IDLE;
      udata.pkt_size = self->rx_received_bytes;
      DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_PKT_END,
                                      udata);
    }
    return 1;
  }
 ...
 *
 * To keep the protocol hardware independant, this function does not call any
 * UART API. It just sends some events to the transport callback. Transport
 * callback is in charge of calling UART driver.
 *
 * If DMA is enabled, function sends DYN_PRO_TRANSPORT_EVENT_TX_START_DMA event
 * and frame pointer to transport callback If DMA is disabled, function sends :
 *    - DYN_PRO_TRANSPORT_EVENT_TX_START event and frame size in bytes to the
 * transport callback
 *    - DYN_PRO_TRANSPORT_EVENT_TX_BYTE event for each byte of the frame
 *    - DYN_PRO_TRANSPORT_EVENT_TX_END event and frame pointer to transport
 * callback
 *
 ...
 */
int DynProTransportUart_txSendFrame(DynProTransportUart_t *self,
  ...
  if (self->use_tx_dma) {
    ...
    DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_TX_START_DMA,
                                    udata);
  } else {
    udata.tx_start = frame->len;
    DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_TX_START,
                                    udata);

    for (i = 0; i < frame->len; ++i) {
      udata.tx_byte = ((uint8_t *)frame->header)[i];
      DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_TX_BYTE,
                                      udata);
    }

    // send frame pointer to callback in case some memory need to be free
    udata.frame = (void *)frame;
    DynProTransportUart_callEventCB(self, DYN_PRO_TRANSPORT_EVENT_TX_END,
                                    udata);
  }
...
/** @deprecated Deprecated function. Consider using
 * DynProTransportUart_txSendFrame() instead.
 *
 ...
 * callback is in charge of calling UART driver.
 *
 * Function starts by sending DYN_PRO_TRANSPORT_EVENT_TX_START event to
 * transport callback. Then it sends DYN_PRO_TRANSPORT_EVENT_TX_BYTE event to
 * the transport callback for each byte of :
 ...
 * @return      0 on sucess, negative value on error
 */
int DynProTransportUart_tx(DynProTransportUart_t *self, const uint8_t *buffer,
                           uint16_t size) {
  union DynProTransportEventData udata;
...
int DynProTransportUart_txEncodeFrame(DynProTransportUart_t *self,
  ...
  if ((frame->payload_len <= 0) ||
      (frame->payload_data !=
       (frame->header + DYN_PRO_TRANSPORT_UART_OVERHEAD))) {
    return -1;
  }

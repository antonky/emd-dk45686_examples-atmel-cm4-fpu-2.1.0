 ...
 * It enables gyroscope at 200Hz and emulates a camera module FSYNC signal at
 * 30Hz to IMU FSYNC pin. Gyroscope data and FSYNC tag can be either read from
 * FIFO or from IMU Sensor registers, depending on user request through UART.
 */
...
static uint8_t
    use_fifo; /* Indicates if FSYNC is tagged in FIFO or in sensor registers */
...
static int setup_mcu() {
  ...
  rc |= si_start_gpio_fsync(
      2 * FSYNC_FREQUENCY_HZ /* FSYNC with rising edge @ 30Hz */,
      ext_fsync_toggle_cb);
...
static int setup_imu() {
  ...
  rc |= inv_imu_adv_set_int2_pin_usage(
      &imu_dev, IOC_PAD_SCENARIO_OVRD_INT2_CFG_OVRD_VAL_FSYNC);
...
static void sensor_event_cb(inv_imu_sensor_event_t *event) {
  ...
  if (event->sensor_mask & (1 << INV_SENSOR_FSYNC_EVENT)) {
    event->timestamp_fsync =
        (uint16_t)((uint64_t)event->timestamp_fsync *
     ...
     */
    // INV_MSG(INV_MSG_LEVEL_INFO, "%u: FSYNC %hd", (uint32_t)event_fsync_ts,
  ...
  if (event->sensor_mask & (1 << INV_SENSOR_GYRO)) {
    ...
    if (timestamp == event_fsync_ts) {
      INV_MSG(INV_MSG_LEVEL_INFO, "%10llu us   %s   FSYNC event %hd", timestamp,
              gyro_str, (uint16_t)event_fsync_delay_cnt);
    } else {
...
static int tag_fsync_in_registers(void) {
  ...
  rc |=
      inv_imu_adv_configure_fsync_ap_tag(&imu_dev, FSYNC_CONFIG0_AP_FSYNC_TEMP);
...
static int print_help() {
  ...
  INV_MSG(INV_MSG_LEVEL_INFO,
          "# 'r' : Get FSYNC flag and counter from IMU Sensor registers");

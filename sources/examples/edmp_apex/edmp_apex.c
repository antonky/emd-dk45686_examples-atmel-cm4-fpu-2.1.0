 ...
 * It automatically enables accelerometer at appropriate frequency and in
 * optimal power mode depending on requested feature combination.
 */
    ...
    if (int1_flag) {
      ...
      if (int_state.INV_EDMP_EVENT) {
        ...
        if (pedometer_en && apex_state.INV_STEP_CNT_OVFL) {
          step_cnt_ovflw++;
          INV_MSG(INV_MSG_LEVEL_INFO, "   %10llu us   STEP_CNT_OVFL",
                  timestamp);
        }
        if (pedometer_en && apex_state.INV_STEP_DET) {
          ...
          if (rc == INV_IMU_OK) {
            snprintf(step_count_str, 100,
                     ...
                     (unsigned long)ped_data.step_cnt +
                         (step_cnt_ovflw * UINT16_MAX),
                     1000000 /
                         ((float)ped_data.step_cadence * 0.25 * ped_odr_us),
          ...
          INV_MSG(INV_MSG_LEVEL_INFO, "   %10llu us   STEP_DET     %s",
                  timestamp, step_count_str);
        }
        ...
        if (tap_en && apex_state.INV_TAP) {
          ...
          if (tap_data.num == INV_IMU_EDMP_TAP_DOUBLE)
            snprintf(tap_str, 80, "%s   %s   duration: %lu us", "Double",
                     tap_dir_str,
                     (unsigned long)tap_data.double_tap_timing * odr_us);
          else
            snprintf(tap_str, 80, "%s   %s", "Single", tap_dir_str);

          INV_MSG(INV_MSG_LEVEL_INFO, "   %10llu us   TAP     %s", timestamp,
                  tap_str);
        }
        ...
        if (ff_en && apex_state.INV_FF) {
          ...
          INV_MSG(INV_MSG_LEVEL_INFO,
                  "   %10llu us   FREEFALL     duration: %u samples (%u ms)",
                  timestamp, duration, (duration * odr_us) / 1000);
        }
...
static int configure_and_enable_edmp_algo() {
  ...
  /* Set accel in low-power mode if ODR slower than 800 Hz, otherwise in
   * low-noise mode */
  ...
  if (smd_en) {
    ...
    rc |= inv_imu_adv_configure_wom(
        &imu_dev, 6, 6, 6, // 6 * 1/256mg ~= 23.44mg for each axis
  ...
  if (pedometer_en || smd_en || tilt_en || r2w_en || tap_en || ff_en ||
      lowg_en || highg_en)
...
static int get_uart_command() {
  ...
  case 'p':
    ...
    INV_MSG(INV_MSG_LEVEL_INFO, "%s Pedometer.",
            pedometer_en ? "Enabling" : "Disabling");
    rc |= configure_and_enable_edmp_algo();
    SI_CHECK_RC(rc);
    break;
  case 's':
  ...
  case 'f':
    ff_en = !ff_en;
    INV_MSG(INV_MSG_LEVEL_INFO, "%s Freefall.",
            ff_en ? "Enabling" : "Disabling");
    rc |= configure_and_enable_edmp_algo();
    SI_CHECK_RC(rc);
    break;
  case 'l':
  ...
  case 'i':
    highg_en = !highg_en;
    INV_MSG(INV_MSG_LEVEL_INFO, "%s HighG.",
            highg_en ? "Enabling" : "Disabling");
    rc |= configure_and_enable_edmp_algo();
    SI_CHECK_RC(rc);
    break;
  case 'c':
...
static int print_current_config() {
  ...
  INV_MSG(INV_MSG_LEVEL_INFO, "#");
  INV_MSG(INV_MSG_LEVEL_INFO, "# Pedometer: %s",
          pedometer_en ? "Enabled" : "Disabled");
  ...
  INV_MSG(INV_MSG_LEVEL_INFO, "#");
  INV_MSG(INV_MSG_LEVEL_INFO, "# ODR: %u Hz",
          (uint32_t)((float)1000000 / odr_us));
  INV_MSG(INV_MSG_LEVEL_INFO, "#");

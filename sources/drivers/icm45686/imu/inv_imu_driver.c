...
int inv_imu_soft_reset(inv_imu_device_t *s) {
  ...
  status |=
      inv_imu_read_reg(s, INTF_CONFIG1_OVRD, 1, (uint8_t *)&intf_config1_ovrd);
  ...
  status |=
      inv_imu_write_reg(s, INTF_CONFIG1_OVRD, 1, (uint8_t *)&intf_config1_ovrd);
...
int inv_imu_set_accel_mode(inv_imu_device_t *s,
...
int inv_imu_set_gyro_mode(inv_imu_device_t *s,
...
int inv_imu_set_accel_frequency(inv_imu_device_t *s,
...
int inv_imu_set_gyro_frequency(inv_imu_device_t *s,
...
int inv_imu_set_accel_fsr(inv_imu_device_t *s,
...
int inv_imu_set_gyro_fsr(inv_imu_device_t *s,
...
int inv_imu_set_gyro_ln_bw(inv_imu_device_t *s,
                           ipreg_sys1_reg_172_gyro_ui_lpfbw_sel_t gyr_bw) {
  int status = INV_IMU_OK;
  ipreg_sys1_reg_172_t ipreg_sys1_reg_172;

  status |= inv_imu_read_reg(s, IPREG_SYS1_REG_172, 1,
                             (uint8_t *)&ipreg_sys1_reg_172);
  ipreg_sys1_reg_172.gyro_ui_lpfbw_sel = gyr_bw;
  status |= inv_imu_write_reg(s, IPREG_SYS1_REG_172, 1,
                              (uint8_t *)&ipreg_sys1_reg_172);

  return status;
}

int inv_imu_set_gyro_lp_avg(inv_imu_device_t *s,
                            ipreg_sys1_reg_170_gyro_lp_avg_sel_t gyr_avg) {
  int status = INV_IMU_OK;
  ipreg_sys1_reg_170_t ipreg_sys1_reg_170;

  status |= inv_imu_read_reg(s, IPREG_SYS1_REG_170, 1,
                             (uint8_t *)&ipreg_sys1_reg_170);
  ipreg_sys1_reg_170.gyro_lp_avg_sel = gyr_avg;
  status |= inv_imu_write_reg(s, IPREG_SYS1_REG_170, 1,
                              (uint8_t *)&ipreg_sys1_reg_170);
  return status;
}

int inv_imu_set_accel_lp_avg(inv_imu_device_t *s,
                             ipreg_sys2_reg_129_accel_lp_avg_sel_t acc_avg) {
  int status = INV_IMU_OK;
  ipreg_sys2_reg_129_t ipreg_sys2_reg_129;

  status |= inv_imu_read_reg(s, IPREG_SYS2_REG_129, 1,
                             (uint8_t *)&ipreg_sys2_reg_129);
  ipreg_sys2_reg_129.accel_lp_avg_sel = acc_avg;
  status |= inv_imu_write_reg(s, IPREG_SYS2_REG_129, 1,
                              (uint8_t *)&ipreg_sys2_reg_129);

  return status;
}

int inv_imu_set_accel_ln_bw(inv_imu_device_t *s,
                            ipreg_sys2_reg_131_accel_ui_lpfbw_t acc_bw) {
  int status = INV_IMU_OK;
  ipreg_sys2_reg_131_t ipreg_sys2_reg_131;

  status |= inv_imu_read_reg(s, IPREG_SYS2_REG_131, 1,
                             (uint8_t *)&ipreg_sys2_reg_131);
  ipreg_sys2_reg_131.accel_ui_lpfbw_sel = acc_bw;
  status |= inv_imu_write_reg(s, IPREG_SYS2_REG_131, 1,
                              (uint8_t *)&ipreg_sys2_reg_131);

  return status;
}

int inv_imu_get_register_data(inv_imu_device_t *s,
...
int inv_imu_set_fifo_config(inv_imu_device_t *s,
  ...
  /* Set fifo WM triggering condition: interrupts when equal or greater than
   * threshold */
  ...
  if ((fifo_config->fifo_mode != FIFO_CONFIG0_FIFO_MODE_BYPASS) &&
      ((fifo_config->accel_en && fifo_config->gyro_en) ||
...
int inv_imu_get_fifo_config(inv_imu_device_t *s,
...
int inv_imu_flush_fifo(inv_imu_device_t *s) {
  ...
  /* Wait for FIFO flush (idle bit will go high at appropriate time and unlock
   * flush) */
...
int inv_imu_get_fifo_frame(inv_imu_device_t *s, inv_imu_fifo_data_t *data) {
  ...
  case 8:
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[1],
                        &data->byte_8.sensor_data[0]);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[3],
                        &data->byte_8.sensor_data[1]);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[5],
                        &data->byte_8.sensor_data[2]);
    data->byte_8.temp_data = frame[7]; // Temp data is only one byte
    break;
  case 16:
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[1],
                        &data->byte_16.accel_data[0]);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[3],
                        &data->byte_16.accel_data[1]);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[5],
                        &data->byte_16.accel_data[2]);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[7],
                        &data->byte_16.gyro_data[0]);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[9],
                        &data->byte_16.gyro_data[1]);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[11],
                        &data->byte_16.gyro_data[2]);
    data->byte_16.temp_data = frame[13]; // Temp data is only one byte
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[14],
                        &data->byte_16.timestamp);
    break;
  case 20:
    ...
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[13],
                        &data->byte_20.temp_data);
    FORMAT_16_BITS_DATA(s->endianness_data, &frame[15],
                        &data->byte_20.timestamp);
    break;
  default:
    data->header.Byte = 0;
...
int inv_imu_set_config_int(inv_imu_device_t *s, const inv_imu_int_num_t num,
  ...
  intx_configx.int1_config1.int1_status_en_i3c_protocol_err =
      it->INV_I3C_PROT_ERR;
...
int inv_imu_select_accel_lp_clk(inv_imu_device_t *s,

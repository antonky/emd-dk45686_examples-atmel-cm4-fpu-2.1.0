...
/* Keep track of when we printed the last event to prevent overloading UART output */
...
void init_frontend(int (*cmd_cb)(inv_commands cmd))
{
	...
	data_to_print = MASK_PRINT_ACC_DATA | MASK_PRINT_GYR_DATA | MASK_PRINT_MAG_DATA |
                 ...
                 const InvnAlgoAGMOutput *output)
{
	...
	/* 
	 * Handle case where last print is in the future (can happen to freeze UART output for some 
	 * time, for instance when printing help.
	 */
	...
	if (delay_since_last_print_us >= print_period_us) {
		...
		if (data_to_print & MASK_PRINT_INPUT_DATA) {
			...
			if (command_cb(IS_MAG_INIT_SUCCESSFUL)) {
				INV_MSG(INV_MSG_LEVEL_INFO, "%lld: INPUT  RMag=[%d, %d, %d]", input->sRmag_time_us,
		...
		if (data_to_print & MASK_PRINT_ACC_DATA) {
			...
			INV_MSG(INV_MSG_LEVEL_INFO,
			        ...
			        input->sRimu_time_us, acc_g[0], acc_g[1], acc_g[2], acc_bias[0] * 1000,
			        acc_bias[1] * 1000, acc_bias[2] * 1000, (int32_t)output->acc_accuracy_flag);
		}
		...
		if (data_to_print & MASK_PRINT_GYR_DATA) {
			...
			INV_MSG(
			    ...
			    input->sRimu_time_us, gyr_dps[0], gyr_dps[1], gyr_dps[2], gyr_bias[0], gyr_bias[1],
			    gyr_bias[2], temp, (int32_t)output->gyr_accuracy_flag);
		}

		/* Mag */
		if (command_cb(IS_MAG_INIT_SUCCESSFUL) && (data_to_print & MASK_PRINT_MAG_DATA)) {
			float mag_cal[3];
			float mag_bias[3];
			...
			INV_MSG(
			    INV_MSG_LEVEL_INFO,
			    "%lld: OUTPUT Mag=[%.3f, %.3f, %.3f]uT MagBias=[%.3f, %.3f, %.3f]uT Accuracy=%d",
			    input->sRmag_time_us, mag_cal[0], mag_cal[1], mag_cal[2], mag_bias[0], mag_bias[1],
			    mag_bias[2], output->mag_accuracy_flag);
		}
		...
		if (data_to_print & MASK_PRINT_6AXIS_DATA) {
			...
			INV_MSG(INV_MSG_LEVEL_INFO, "%lld: OUTPUT Euler6Axis=[%.2f, %.2f, %.2f]deg",
			        input->sRimu_time_us, angles_deg_grv[0], angles_deg_grv[1], angles_deg_grv[2]);
		}

		if (command_cb(IS_MAG_INIT_SUCCESSFUL)) {
			float rv_heading_accuracy = RAD_TO_DEG((float)output->rv_accuracy_q27 / (1 << 27));
			...
			if (data_to_print & MASK_PRINT_9AXIS_DATA) {
				...
				INV_MSG(INV_MSG_LEVEL_INFO,
				        ...
				        input->sRimu_time_us, rv_quat[0], rv_quat[1], rv_quat[2], rv_quat[3],
				        rv_heading_accuracy);

				INV_MSG(INV_MSG_LEVEL_INFO,
				        "%lld: OUTPUT Euler9Axis=[%.2f, %.2f, %.2f]deg 9AxisAccuracy=[%f]deg",
				        input->sRimu_time_us, angles_deg_rv[0], angles_deg_rv[1], angles_deg_rv[2],
				        rv_heading_accuracy);
			}
		}
...
static void print_help(void)
{
	...
	if (command_cb(IS_MAG_INIT_SUCCESSFUL)) {
		...
		INV_MSG(INV_MSG_LEVEL_INFO, "# '9' : print 9-axis quaternion and eulers angles (RV)");
	}
	INV_MSG(INV_MSG_LEVEL_INFO, "# '6' : print 6-axis quaternion and eulers angles (GRV)");
	...
	INV_MSG(INV_MSG_LEVEL_INFO, "# 'f' : configure printing period to 'fast' (20 ms)");
	INV_MSG(INV_MSG_LEVEL_INFO, "# 's' : configure printing period to 'slow' (1 s)");

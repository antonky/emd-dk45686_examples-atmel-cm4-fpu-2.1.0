...
/*
 * This example showcases how to retrieve data from the registers using the
 * basic driver. It prints accelerometer and/or gyroscope data in either SI unit
 * or in raw data format. Sensors Low-Power/Low-Noise can be configured. It
 * starts accelerometer and gyroscope in Low-Noise mode at 50Hz by default.
 */
  ...
  do {
    ...
    if (int1_flag) {
      ...
      if (int_state.INV_UI_DRDY) {
         ...
         * function). Temperature is coded as 16-bits signed with a scale factor
        ...
        if (print_lsb) {
          ...
          if (accel_en && (discard_accel_samples == 0) &&
              ...
              (d.accel_data[1] != INVALID_VALUE_FIFO) &&
              (d.accel_data[2] != INVALID_VALUE_FIFO))
            snprintf(accel_str, 40, "Accel:% 8d % 8d % 8d",
                     (int)d.accel_data[0], (int)d.accel_data[1],
                     (int)d.accel_data[2]);
          else
            snprintf(accel_str, 40, "Accel:       -        -        -");
          ...
          INV_MSG(INV_MSG_LEVEL_INFO, "LSB %10llu us   %s     %s       %s",
                  timestamp, accel_str, gyro_str, temp_str);
        }
...
static int setup_imu() {
  ...
  /* Sensor registers are not available in ULP, so select RCOSC clock to use LP
   * mode. */

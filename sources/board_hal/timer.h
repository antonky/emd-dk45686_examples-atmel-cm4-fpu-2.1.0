 ...
 *  @param	timer_num  Timer peripheral number
 *  @return timestamp  timestamp value in us on 64-bits for INV_TIMER1, 16-bits
 * otherwise
 */
uint64_t inv_timer_get_counter(unsigned timer_num);

/** @brief Configure a timer which will trigger a callback at the end of its
 * period
 ...
 *  @return -1 if parameters are not configurable, the timer's channel on which
 * the callback is registered on success
 */
...
/** @brief Disable the specified timer's channel without further
 * re-initialisation
 *  @param[in]	timer_num  Timer peripheral number
 ...
 *  reconfigured (deadline, resolution, ..) but only have its channel put on
 * hold. This function is not expected to cleanly reset timer resource (it is
 * not a duplicate of \sa timer_channel_stop, which shall be called to release
 * the resource).
 *  @note \sa timer_configure_callback should not be called directly after this
 * function.
 */

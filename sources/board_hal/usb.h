...
typedef enum usb_num {
  INV_USB_TARGET, /**< Just a fancier and hw independant name for USB connector.
                     ...
                     < On samg55 it is mapped on J301 USB port with PA21_USB_N /
                     PA22_USB_P lines. */
} inv_usb_num_t;

/** @brief USB initialisation structure.
 *
 * Such a structure should be filled-in and passed to the usb_init function in
 * order to initialize the usb peripheral.
 */
typedef struct inv_usb_init_struct {
  inv_usb_num_t usb_num; /** < USB peripheral to initialize */
  void (*suspend_action_cb)(
      void); /** < Callback executed when USB bus is wakeup */
  void (*resume_action_cb)(
      void); /** < Callback executed for each received SOF */

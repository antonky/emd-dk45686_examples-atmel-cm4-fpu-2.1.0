 ...
 * incremented. Low values reduce latency but increase false positives. High
 * values increase step count accuracy but increase latency Unit: Number of
 * steps Range: [0-15] Default: 5
 */
 ...
 *
 * Intermediate number of steps needed to be buffered waiting the internal
 * counting reached the ped_step_cnt_th (when ped_prev_step_cnt_th is lower than
 * ped_step_cnt_th). Note: as soon as ped_step_cnt_th is reached
 * ped_prev_step_cnt_th value is set to ped_step_cnt_th. Unit: number of steps
 * Range: [0-15]
 * Default value: 5
 ...
 * While in the step buffer state, the step buffer count resets to 0 if a new
 * step isn't detected for this amount of time (user is considered to have
 * "stopped walking") Unit: time in samples number Range: [0 - 225] Default
 * value: 150 for ODR = 50 Hz Recommendation: it is linked to stop and go use
 * case to avoid seeing the impact of pedo_step_cnt_thr and pedo_step_det_thr
 */
 ...
 * Slow walk mode improves slow walk detection (less than 1 Hz) but the number
 * of false positives may increase Range: 0: Normal 1: Slow walk Default: 0
 */
 ...
 * walk use case only. Unit: g in q25 Range: [1006632 - 3523215] Default:
 * 2684354
 */
 ...
 * Unit: 4*number of samples between two consecutive steps. Cadency (step/s) =
 * (ped_step_cadence / 4) / (pedometer_ ODR).
 */
 ...
 * event. Unit: time in sample number Range: [0 - 65536] Default: 200 for ODR =
 * 50 Hz, 100 for ODR = 25 Hz
 */
 ...
 * applicable only if tilt_reset_en is also set to 1. Unit: N/A Range: [0 - 1]
 * Default: 0
 */
...
/* smd_sensitivity
 *
 * Parameter to tune SMD algorithm robustness to rejection, ranging from 0 to 4
 * (values higher than 4 are reserved). Low values increase detection rate but
 * increase false positives. High values reduce false positives but reduce
 * detection rate (especially for transport use cases). Unit: N/A Ranging: [0 -
 * 4] Default: 0
 */
...
/* basicsmd_win
 *
 * The window time in number of samples to wait continuous WOM before trigger
 * SMD Unit: time in sample number Range: [25 - 500] Default value: 150
...
/* basicsmd_win_wait
 *
 * The window time in number of samples before stop trigger SMD after WOM stop
 * reported motion Unit: time in sample number Range: [25 - 500] Default value:
 * 50 Recommended value at 50Hz = 50 Recommended value at 25Hz = 25
 */
 ...
 * position changes or not. Unit: time in ms (millisecond) Range: [100 - 10000]
 ...
 * event. Unit: time in ms (millisecond) Range: [0 - 256] Default: 96
 * (equivalent to 0.096 s)
 */
 ...
 * (low-pass filter), defined as : forgetting factor = Gain * SAMPLING_PERIOD /
 * (40 * 32), and 100Hz. Range: [2-16] Default: 6 for ODR = 50Hz, 8 for ODR =
 * 25Hz
 */
 ...
 * Range: [130856211 - 1069655912], corresponding to angle between 5 and 85
 * degrees Default: 1046221864, corresponding to an angle of 13 degrees
 */
 ...
 * inclination). Unit: ms (no dependency on ODR, it is managed internally by the
 * algorithm) Range: [100 - 500] Default: 240
 */
 ...
 *
 * Timer relative to the rapidity of the algorithm to trigger wake up when the
 * orientation before motion is over 30 degrees on the Y axis Unit: ms (no
 * dependency on ODR, it is managed internally by the algorithm) Range: [240-
 * 1000] Default: 500
 */
 ...
 * detected. Longer time enables detection motion during slower gesture. Unit:
 * ms (no dependency on ODR, it is managed internally by the algorithm) Range:
 ...
 * detected. Unit: ms (no dependency on ODR, it is managed internally by the
 * algorithm) Range: [100 - 1000] Default: 480
 */
 ...
 *
 * Time delay to update gravity in case motion is detected all the time, force
 * to update gravity estimator even if the device is not stable. Unit: ms (no
 * dependency on ODR, it is managed internally by the algorithm) Range: [1000 -
 * 10000] Default: 2600
 */
...
/* lowg_peak_th_hyst
 *
 * Hysteresis value added to the low-g threshold after accel values get below
 * threshold. Unit: g in q12 Range: [128 - 1024] Default: 128
 */
...
/* highg_time_th
 *
 * The number of samples device should stay above (highg_peak_th +
 * highg_peak_th_hyst) before HighG state is triggered. Unit: time in samples
 * number Range: [1-300] Default: 1 (set for default ODR = 800 Hz, equivalent
 * to 1.25 ms)
 */
 ...
 * detected. Prevents false detection due to bounces. Unit: time in samples
 * number Range: [75 - 3000] Default: 800 (set for default ODR = 800 Hz,
 * equivalent to 1 s)
 */
 ...
 * Unit: number of samples. Freefall duration in seconds is ff_duration_buf1 /
 * ACCEL_ODR_Hz
 */
 ...
 * Unit: number of samples. Freefall duration in seconds is ff_duration_buf2 /
 * ACCEL_ODR_Hz
 */
...
/* tap_max_peak_tol
 *
 * Maximum peak tolerance is the percentage of pulse amplitude to get the smudge
 * threshold for rejection. Unit: N/A Range: [1 (12.5%) 2 (25.0%) 3 (37.5%) 4
 * (50.0 %)] Default: 2
 */
 ...
 *
 * max acceptable number of samples (jerk value) over  tap_max_peak_tol the
 * during the Tmin window. Over this value, Tap event is rejected unit: time in
 * number of samples range: [13 â€“ 92] Default: 17 (set for default ODR = 200
 * Hz, equivalent to 0.085 s)
 */
...
/* tap_tavg
 *
 * Energy measurement window size to determine the tap axis associated with the
 * 1st tap. Unit: time in sample number Range: [1 ; 2 ; 4 ; 8] Default: 8
 *
 */
...
/* double_tap_timing
 *
 * In case of double tap, indicate the sample count between the two detected
 * pulses. Double tap timing in seconds is double_tap_timing / ACCEL_ODR_Hz.
 */
...
/* es0_compass_en
 *
 * Set 1 for compass support with es0, for other external sensors with es0, set
 * 0.
 */
...
/* es_power_mode
 *
 * Set 1 for possibility of power savings when APEX features utilization is
...
/* es_ram_image_en
 *
 * Set 1 to load device specific RAM image for external sensor support,
 * otherwise set 0
 */
 ...
 * bit0: If set, enable self-test init, must be set when any of accel or gyro
 * self-test enable bit is set (bits 2:1) bit1: If set, enable accel self-test
 * bit2: If set, enable gyro self-test
 * bit7~9: Averaging time used to perform self-test (0/1/2/3/4/5:
 * 10/20/40/80/160/320 ms) bit10~12: Tolerance between factory trim and accel
 * self-test response (0/1/2/3/4/5/6/7: 5/10/15/20/25/30/40/50%) bit13~15:
 * Tolerance between factory trim and gyro self-test response (0/1/2/3/4/5/6/7:
...
/* stc_debug_en
 *
 * Debug capability of self-test feature. Must be set to 0 at anytime when
 * self-test is requested.
 */

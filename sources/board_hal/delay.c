...
static void internal_delay(uint16_t us) {
  ...
  uint32_t end;
  uint32_t prev;
  now = tc_read_cv(delay_timer_instance.timer_instance,
                   delay_timer_instance.timer_channel);
  end = now + us;
  prev = now;
...
int inv_delay_init(unsigned tim_num) {
  ...
  if (INV_TIMER1 == tim_num) {
    ...
    rc = inv_timer_get_hw_infos(tim_num, &delay_timer_instance.timer_instance,
                                &delay_timer_instance.timer_channel);
...
void inv_delay_us(uint32_t us) {
  if ((tc_get_status(delay_timer_instance.timer_instance,
                     delay_timer_instance.timer_channel) &
       TC_SR_CLKSTA) == 0) {
    tc_start(delay_timer_instance.timer_instance,
             delay_timer_instance.timer_channel);
    while ((tc_get_status(delay_timer_instance.timer_instance,
                          delay_timer_instance.timer_channel) &
            TC_SR_CLKSTA) == 0) {
    }
  }
  ...
  if (us >= UINT16_MAX) {
    /* go to the loop as the internal_delay function only support uint16_t
     * argument type */
...
void inv_delay_ms(uint32_t ms) { inv_delay_us(ms * 1000); }

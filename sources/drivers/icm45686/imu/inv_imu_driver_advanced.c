...
int inv_imu_adv_disable_accel(inv_imu_device_t *s) {
   ...
   * If it is, set `clear_fifo` flag to reset fifo after accel is actually
   * turned off.
   */
...
int inv_imu_adv_disable_gyro(inv_imu_device_t *s) {
   ...
   * If it is, set `clear_fifo` flag to reset fifo after gyro is actually turned
   * off.
   */
...
int inv_imu_adv_set_int2_pin_usage(
    inv_imu_device_t *s, ioc_pad_scenario_ovrd_pads_int2_cfg_ovrd_val_t usage) {
  int status = INV_IMU_OK;
  ...
  status |= inv_imu_read_reg(s, IOC_PAD_SCENARIO_OVRD, 1,
                             (uint8_t *)&ioc_pad_scenario_ovrd);
  ioc_pad_scenario_ovrd.pads_int2_cfg_ovrd = 1;
  ioc_pad_scenario_ovrd.pads_int2_cfg_ovrd_val = (uint8_t)usage;
  status |= inv_imu_write_reg(s, IOC_PAD_SCENARIO_OVRD, 1,
                              (uint8_t *)&ioc_pad_scenario_ovrd);
...
#if INV_IMU_FSYNC_SUPPORTED
int inv_imu_adv_configure_fsync_ap_tag(
    inv_imu_device_t *s, fsync_config0_ap_fsync_sel_t sensor_tag) {
  int status = INV_IMU_OK;
  inv_imu_adv_var_t *e = (inv_imu_adv_var_t *)s->adv_var;
...
int inv_imu_adv_enable_fsync(inv_imu_device_t *s) {
  ...
  status |=
      inv_imu_read_reg(s, TMST_WOM_CONFIG, 1, (uint8_t *)&tmst_wom_config);
  tmst_wom_config.tmst_delta_en = INV_IMU_ENABLE;
  status |=
      inv_imu_write_reg(s, TMST_WOM_CONFIG, 1, (uint8_t *)&tmst_wom_config);
...
int inv_imu_adv_configure_wom(inv_imu_device_t *s, const uint8_t wom_x_th,
...
int inv_imu_adv_get_data_from_registers(inv_imu_device_t *s) {
  ...
  FORMAT_16_BITS_DATA(s->endianness_data, &data[0],
                      (uint16_t *)&event.accel[0]);
  FORMAT_16_BITS_DATA(s->endianness_data, &data[2],
                      (uint16_t *)&event.accel[1]);
  FORMAT_16_BITS_DATA(s->endianness_data, &data[4],
  ...
  FORMAT_16_BITS_DATA(s->endianness_data, &data[10],
  ...
  /* Temperature */
  FORMAT_16_BITS_DATA(s->endianness_data, &data[12],
  ...
  /* Sensor data register is configured to expose fsync flag. If fsync flag is
   * set, process fsync counter. */
  if (fsync_tag_in_accel || fsync_tag_in_gyro || fsync_tag_in_temp) {
    uint8_t fsync_count[2];

    /*
     * Read 16bits fsync counter containing time elapsed between last FSYNC
     * interrupt and last ODR event. Fsync delta time depends on data endianness
     * as counter is read over 2 registers and timestamp resolution.
     */
 ...
 */
static int decode_compressed_event(inv_imu_device_t *s, uint8_t *frame,
                                   uint8_t event_num) {
  int status = INV_IMU_OK;
  inv_imu_adv_var_t *e = (inv_imu_adv_var_t *)s->adv_var;
  const fifo_comp_header_t *comp_header = (const fifo_comp_header_t *)&frame[0];
  const fifo_comp_decode_t *decode_header =
      (const fifo_comp_decode_t *)&frame[1];
  ...
  /*
   * Temperature
   * In compressed frames, temperature is only available if accel and gyro are
   * enabled
   */
  event.temperature = INVALID_VALUE_FIFO_1B;
  if (comp_header->bits.accel_bit &&
      comp_header->bits.gyro_bit) /* accel_en + gyro_en */ {
    if (((decode_header->bits.valid_samples_a & validity_mask) ||
...
static int parse_compressed_fifo_frame(inv_imu_device_t *s, uint8_t *frame) {
  ...
  for (uint8_t i = FIFO_COMP_1_SAMPLE_IN_FRAME; i <= header->bits.tot_sample;
       i++) {
 ...
 *         Applies for uncompressed frames generated when FIFO compression is
 * enabled.
 *  @param[in] s      Pointer to device.
 *  @param[in] frame  Data to parse.
 */
...
int inv_imu_adv_get_data_from_fifo(inv_imu_device_t *s,
   ...
   * interrupt is triggered with M number of FIFO frames accumulated in the FIFO
   * buffer, the host should only read the first M-1 number of FIFO frames
   */
  if (e->fifo_mode == FIFO_CONFIG0_FIFO_MODE_STREAM)
    (*fifo_count)--;

  /* Read FIFO data */
  status |= inv_imu_read_reg(s, FIFO_DATA, *fifo_count * s->fifo_frame_size,
                             fifo_data);
  return status;
}

int inv_imu_adv_parse_fifo_data(inv_imu_device_t *s,
...
int inv_imu_adv_get_accel_fsr(inv_imu_device_t *s,
  ...
  if (e->fifo_is_used == INV_IMU_ENABLE &&
      (fifo_config3.fifo_hires_en == INV_IMU_ENABLE)) {
...
int inv_imu_adv_get_gyro_fsr(inv_imu_device_t *s,
...
int inv_imu_adv_set_timestamp_resolution(
    inv_imu_device_t *s, const tmst_wom_config_tmst_resol_t timestamp_resol) {
  int status = INV_IMU_OK;
  tmst_wom_config_t tmst_wom_config;
...
int inv_imu_adv_get_fifo_config(inv_imu_device_t *s,
  ...
  /* FIFO_CONFIG0 */
  conf->base_conf.fifo_mode =
      (fifo_config0_fifo_mode_t)cfg.fifo_config0.fifo_mode;
  if (cfg.fifo_config0.fifo_depth == FIFO_CONFIG0_FIFO_DEPTH_MAX ||
    ...
    conf->base_conf.fifo_depth =
        (fifo_config0_fifo_depth_t)cfg.fifo_config0.fifo_depth;
  } else {
    /*
  ...
  /* FIFO_CONFIG2 */
  conf->fifo_wr_wm_gt_th =
      (fifo_config2_fifo_wr_wm_gt_th_t)cfg.fifo_config2.fifo_wr_wm_gt_th;
  ...
  /* FIFO_CONFIG4 */
  conf->comp_nc_flow_cfg = (fifo_config4_fifo_comp_nc_flow_cfg_t)
                               cfg.fifo_config4.fifo_comp_nc_flow_cfg;
  conf->comp_en = cfg.fifo_config4.fifo_comp_en;
  conf->tmst_fsync_en = cfg.fifo_config4.fifo_tmst_fsync_en;
  conf->es0_6b_9b =
      (fifo_config4_fifo_es0_6b_9b_t)cfg.fifo_config4.fifo_es0_6b_9b;

  status |= inv_imu_read_reg(s, ODR_DECIMATE_CONFIG, 1,
                             (uint8_t *)&odr_decimate_config);

  /* ODR_DECIMATE_CONFIG */
  conf->gyro_dec = (odr_decimate_config_gyro_fifo_odr_dec_t)
                       odr_decimate_config.gyro_fifo_odr_dec;
  conf->accel_dec = (odr_decimate_config_accel_fifo_odr_dec_t)
                        odr_decimate_config.accel_fifo_odr_dec;

  return status;
}

int inv_imu_adv_set_fifo_config(inv_imu_device_t *s,
  ...
  if (cfg.fifo_config4.fifo_comp_en == INV_IMU_ENABLE) {
    ...
    if (pwr_mgmt0.accel_mode != PWR_MGMT0_ACCEL_MODE_OFF) {
      accel_config0_t accel_config0;
      status |=
          inv_imu_read_reg(s, ACCEL_CONFIG0, 1, (uint8_t *)&accel_config0);
      accel_odr =
          inv_imu_adv_convert_odr_bitfield_to_us(accel_config0.accel_odr);
    }
    ...
    status |=
        inv_imu_write_reg(s, FIFO_CONFIG4, 1, (uint8_t *)&cfg.fifo_config4);
  ...
  status |= inv_imu_write_reg(s, ODR_DECIMATE_CONFIG, 1,
                              (uint8_t *)&odr_decimate_config);
  ...
  if (conf->base_conf.fifo_mode == FIFO_CONFIG0_FIFO_MODE_BYPASS) {
    ...
    cfg.fifo_config3.fifo_if_en = INV_IMU_DISABLE;
    status |=
        inv_imu_write_reg(s, FIFO_CONFIG3, 1, (uint8_t *)&cfg.fifo_config3);
    /* `fifo_mode` */
    status |=
        inv_imu_write_reg(s, FIFO_CONFIG0, 1, (uint8_t *)&cfg.fifo_config0);
...
int inv_imu_adv_enable_clkin_rtc(inv_imu_device_t *s) {
  ...
  /* Both I3CSM STC and CLKIN use the same interpolator. I3CSM STC has higher
   * priority over the CLKIN. To use the CLKIN, user must disable the I3CSM STC
   * by setting the I3C_STC_MODE = 0
   */
  status |=
      inv_imu_read_reg(s, SIFS_I3C_STC_CFG, 1, (uint8_t *)&sifs_i3c_stc_cfg);
  sifs_i3c_stc_cfg.i3c_stc_mode = INV_IMU_DISABLE;
  status |=
      inv_imu_write_reg(s, SIFS_I3C_STC_CFG, 1, (uint8_t *)&sifs_i3c_stc_cfg);

  /* Same as in I3CSM STC mode, the ACCEL_SRC_CTRL[1:0] and GYRO_SRC_CTRL[1:0]
   * must be set to 2’b10 (FIR and interpolator on)
   */
  status |= inv_imu_read_reg(s, IPREG_SYS2_REG_123, 1,
                             (uint8_t *)&ipreg_sys2_reg_123);
  ipreg_sys2_reg_123.accel_src_ctrl =
      IPREG_SYS2_REG_123_ACCEL_SRC_CTRL_INTERPOLATOR_ON_FIR_ON;
  status |= inv_imu_write_reg(s, IPREG_SYS2_REG_123, 1,
                              (uint8_t *)&ipreg_sys2_reg_123);

  status |= inv_imu_read_reg(s, IPREG_SYS1_REG_166, 1,
                             (uint8_t *)&ipreg_sys1_reg_166);
  ipreg_sys1_reg_166.gyro_src_ctrl =
      IPREG_SYS1_REG_166_GYRO_SRC_CTRL_INTERPOLATOR_ON_FIR_ON;
  status |= inv_imu_write_reg(s, IPREG_SYS1_REG_166, 1,
                              (uint8_t *)&ipreg_sys1_reg_166);

  status |=
      inv_imu_read_reg(s, RTC_CONFIG, 1, (uint8_t *)&otp_heater_rtc_config);
  otp_heater_rtc_config.rtc_mode = INV_IMU_ENABLE;
  status |=
      inv_imu_write_reg(s, RTC_CONFIG, 1, (uint8_t *)&otp_heater_rtc_config);
...
int inv_imu_adv_set_endianness(inv_imu_device_t *s,
...
static int configure_serial_interface(inv_imu_device_t *s) {
  ...
  case UI_SPI4:
    ...
    status |= inv_imu_write_reg(s, INTF_CONFIG1_OVRD, 1,

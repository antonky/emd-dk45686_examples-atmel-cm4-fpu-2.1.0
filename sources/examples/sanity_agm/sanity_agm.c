...
/*
 * This example aims at verifying that the AGM library is properly working by replaying a
 * test vector and verifying the algo generates an identical output.
 * Two test vectors are provided but due to memory limitation, only one can be activated (testing
 * the second one will require a rebuild).
 */
...
static int check_output(int32_t expected_output, int32_t output, const char *str, int line);
...
int main(void)
{
	...
	for (uint32_t i = 0; i < (sizeof(test_vector) / (NUM_COLUMN * sizeof(int32_t))); i++) {
		...
		/* Check output */
		rc |= check_output(expected_output.rv_quat_q30[0], output.rv_quat_q30[0], "rv_quat_q30[0]",
		                   i);
		rc |= check_output(expected_output.rv_quat_q30[1], output.rv_quat_q30[1], "rv_quat_q30[1]",
		                   i);
		rc |= check_output(expected_output.rv_quat_q30[2], output.rv_quat_q30[2], "rv_quat_q30[2]",
		                   i);
		rc |= check_output(expected_output.rv_quat_q30[3], output.rv_quat_q30[3], "rv_quat_q30[3]",
		                   i);

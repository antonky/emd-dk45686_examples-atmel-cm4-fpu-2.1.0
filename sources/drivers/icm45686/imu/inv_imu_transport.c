...
static int write_dreg(inv_imu_transport_t *t, uint8_t reg, uint32_t len,
                      const uint8_t *buf);
static int read_dreg(inv_imu_transport_t *t, uint8_t reg, uint32_t len,
                     uint8_t *buf);

/* For Mclk REGisters */
static int check_out_of_bounds_mreg(uint32_t reg, uint32_t len);
static int write_mreg(inv_imu_transport_t *t, uint32_t reg, uint32_t len,
                      const uint8_t *buf);
static int read_mreg(inv_imu_transport_t *t, uint32_t reg, uint32_t len,
                     uint8_t *buf);
...
static int check_out_of_bounds_mreg(uint32_t reg, uint32_t len) {
  ...
  /* AN-000364
   * Users must not access the following register map address space to prevent
   * stalling the device.
   * - From 0x000023FF to 0x00003FFF
   * - From 0x000083FF to 0x00009FFF
   * - From 0x0000AFFF to 0xFFFFFFFF
   * If user happens to access this space, soft reset is needed after the access

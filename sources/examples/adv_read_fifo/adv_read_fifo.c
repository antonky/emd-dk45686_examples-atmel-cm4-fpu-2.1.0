...
/*
 * This example showcases how to retrieve data from the FIFO using the advanced
 driver.
 * It prints accelerometer and/or gyroscope data in either SI unit or in raw
 data format.
 * High-resolution mode can be enabled and sensors Low-Power/Low-Noise can be
 configured.
 ...
 * When operating in FIFO streaming mode, if FIFO threshold interrupt is
 triggered with M number
 * of FIFO frames accumulated in the FIFO buffer, the host should only read the
 first M-1 number
 * of FIFO frames. This prevents the FIFO empty event, that can cause FIFO data
 corruption, from
 * happening.
 */
...
static uint64_t
    accel_start_time_us;            /* To discard samples when enabling accel */
...
static void sensor_event_cb(inv_imu_sensor_event_t *event) {
   ...
   * 2^(20-1) = 524288) with max FSR (32 g and 4000 dps). Temperature is coded
   * as 16-bits signed with a scale factor of 128 and an offset of 25 °C.
   * Otherwise, accel and gyro data are coded as 16-bits signed (max_lsb =
   * 2^(16-1) = 32768) with the configured FSR (4 g and 2000 dps, see
   * `setup_imu()` function). Temperature is coded as 8-bits signed with a scale
   * factor of 2 and an offset of 25 °C.
   */
  ...
  temp_degc =
      (float)temp_offset_c + ((float)event->temperature / temp_scale_factor);

...
int inv_ak0991x_poll_data(struct inv_ak0991x *s, int16_t *compass_data) {
  ...
  if (s->compass_en) {
    ...
    /* data is ready, read relevant registers now. Reading data registers will
     * reset DRDY bit in ST1 register
     */
    rc = inv_ak0991x_serif_read_reg(&s->serif, AK0991x_REG_HXL, &data[1], 8);
    if (rc != 0) {
      /* for some reason we could not read data register but DRDY has already
       * been fired. Thus it is safer to reset compass_en flag to avoid putting
       * AKM driver in some undefined state.
       */
      s->compass_en = 0;
      return rc;
    }
    ...
    if (s->compass_sens[0] && s->compass_sens[1] && s->compass_sens[2]) {
      ...
      if (INV_AK09911_ID == s->compass_id)
        shift = 7;
      compass_data[0] =
          ((compass_data[0] * (s->compass_sens[0] + 128)) >> shift);
      compass_data[1] =
          ((compass_data[1] * (s->compass_sens[1] + 128)) >> shift);
      compass_data[2] =
          ((compass_data[2] * (s->compass_sens[2] + 128)) >> shift);
    }
...
int inv_ak0991x_soft_reset(struct inv_ak0991x *s) {
  ...
  s->compass_en = 0;
  return inv_ak0991x_serif_write_reg(&s->serif, AK0991x_REG_CNTL3, &soft_reset,
                                     1);
}
...
int inv_ak0991x_retrieve_asa_values(struct inv_ak0991x *s) {
  ...
  if (s->compass_id == INV_AK09918_ID) {
    ...
    rc = inv_ak0991x_serif_read_reg(&s->serif, AK0991x_REG_ASAX,
                                    &s->compass_sens[0], 3);
    if (rc != 0)
      return rc;
    mode = AK0991x_CNTL2_MODE_POWER_DOWN;

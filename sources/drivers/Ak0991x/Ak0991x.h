...
#define AK0991x_CNTL2_MODE_CONTINUOUS1                                         \
  0x02 /* Continuous measurement mode 1 10Hz */
#define AK0991x_CNTL2_MODE_CONTINUOUS2                                         \
  0x04 /* Continuous measurement mode 2 20Hz */
#define AK0991x_CNTL2_MODE_CONTINUOUS3                                         \
  0x06 /* Continuous measurement mode 3 50Hz */
#define AK0991x_CNTL2_MODE_CONTINUOUS4                                         \
  0x08 /* Continuous measurement mode 4 100Hz */
#define AK0991x_CNTL2_MODE_CONTINUOUS5                                         \
  0x0A /* Continuous measurement mode 5 200Hz */
#define AK0991x_CNTL2_MODE_CONTINUOUS6                                         \
  0x0C /* Continuous measurement mode 6 1HZ */
...
/** @brief Hook for low-level system time() function to be implemented by upper
 * layer
...
/** @brief Hook used for accurate timestamping, called by low-level driver after
 * acquisition is complete
 *  @return timestamp to be reported with the last data
 *  @details
 *  The @see inv_ak0991x_get_time_us function is used to obtain the current
 * time, this function has the slightly different purpose of marking the end of
 * the sampling very accurately (relying on timers to achieve this). Host
 * implementation will do the same as @see inv_ak0991x_get_time_us.
 */
extern uint64_t inv_ak0991x_get_dataready_interrupt_time_us(void);

/** @brief Hook called when magnetometer is started, implemented by upper layer
 * which knows about timers
 *  @param[in] period sensor period in micro-second
 *  @return 0 on success, -1 on failure
 */
extern int inv_ak0991x_start_timer_us(uint32_t period);

/** @brief Hook called when magnetometer is stopped, implemented by upper layer
 * which knows about timers
 *  @return 0 on success, -1 on failure
 */
extern int inv_ak0991x_stop_timer(void);

/** @brief Hook called when magnetometer odr is updated, implemented by upper
 * layer which knows about timers
 *  @param[in] period sensor period in micro-second
 *  @return 0 on success, -1 on failure
 */

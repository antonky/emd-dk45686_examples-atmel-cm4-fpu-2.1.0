...
#define MAG_SAMPLING_TIMER INV_TIMER3 /* Used for si_io_akm_init_timer_getdata() */
                                   ...
                                   void interrupt_timer_mag_cb(void *context), int *ch)
{
	...
	channel =
	    inv_timer_configure_callback(MAG_SAMPLING_TIMER, frequency, 0, interrupt_timer_mag_cb);
                               ...
                               int *ch)
{
	...
	/* Configure the timer used to trigger new magnetometer data capture */
	channel = inv_timer_configure_callback(MAG_DATA_TIMER, frequency, 0, interrupt_timer_mag_cb);
...
int si_uninit_clkin()
{
	return inv_gpio_disable_clk_on_pin(INV_GPIO_CLKIN);
}
...
int si_start_gpio_fsync(uint32_t freq, void (*fsync_timer_cb)(void *context))
{
	...
	if (freq) {
		 ...
		 */
		fsync_channel = inv_timer_configure_callback(FSYNC_TIMER, freq, 0, fsync_timer_cb);
		if (fsync_channel < 0)
			rc |= fsync_channel;
	}
...
{
	inv_gpio_toggle_pin(INV_GPIO_INT2);
}

...
/** @brief Enumeration of the SPIs connected to the INV sensor:
           ...
           - INV_SPI_ONBOARD_REVB represents the SPI between AP and on-board ICM
   on rev B
 ...
 */
unsigned long
inv_spi_master_write_register(unsigned spi_num, unsigned char register_addr,
                              unsigned short register_len,
                              const unsigned char *register_value);

/** @brief For systems that send a leading zero byte with each read, set the
 * number of zero bytes.
 * @param[in] spinum, required spi line number
 * @param[in] num_zero_bytes, The number of leading zero bytes, valid options
 * are 0 or 1.
 */

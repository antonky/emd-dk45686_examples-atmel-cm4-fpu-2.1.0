 ...
 * Selftest for accelerometer and gyroscope can be enabled independently through
 * UART. Selftest procedure is run once every second.
 */
      ...
      if (st_params.gyro_en == 0 && st_params.accel_en == 0) {
        ...
        if (rc < 0) {
          INV_MSG(INV_MSG_LEVEL_ERROR,
                  "An error occured while running selftest");
        } else {
